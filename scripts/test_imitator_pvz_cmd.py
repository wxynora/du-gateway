...
class ImitatorPvzCmdTests(unittest.TestCase):
    ...
    def test_cmd_setup_mentions_optional_special_mode(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            cmd_engine.DEFAULT_SAVE_PATH = Path(tmpdir) / "save.json"

            output = cmd_engine.cmd("new_game level=1 seed=setup-special-mode")

            self.assertIn("请先编辑卡槽", output)
            self.assertIn("模式: 默认普通；特殊无尽用 new_game mode=特殊 chaos=off|airdrop（固定六个模仿者）。", output)
            self.assertIn("提示: 模仿者越多，随机味越足。", output)

    def test_cmd_can_start_six_imitator_special_mode(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            cmd_engine.DEFAULT_SAVE_PATH = Path(tmpdir) / "save.json"
    ...
    def test_note_in_setup_does_not_start_engine(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cmd_engine.cmd("new_game level=1 seed=setup-note-test")
            note_output = cmd_engine.cmd("note 开局多带模仿者")
            open_output = cmd_engine.cmd("打开")
            notes_output = cmd_engine.cmd("note")

            self.assertIn("复盘已记录", note_output)
            self.assertIn("请先编辑卡槽", open_output)
            self.assertIn("开局多带模仿者", notes_output)
