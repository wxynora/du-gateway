...
SERVICE_NAME="${SERVICE_NAME:-du-watch-analysis-worker}"
REPO_ROOT="${REPO_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
PYTHON_BIN="${PYTHON_BIN:-$REPO_ROOT/.venv/bin/python}"
SERVICE_FILE="/etc/systemd/system/${SERVICE_NAME}.service"

if [ "$(id -u)" -ne 0 ]; then
  echo "请用 sudo 运行：sudo bash scripts/install_watch_analysis_worker_service.sh" >&2
  exit 1
fi
...
"$PYTHON_BIN" - <<'PY'
from scripts.run_watch_analysis_worker import watch_analysis_source_health

health = watch_analysis_source_health()
if health["enabled"] and not health["ready"]:
    raise SystemExit(f"一起看后端分析源依赖未就绪: {health}")
PY

cat >"$SERVICE_FILE" <<EOF
[Unit]
Description=Du Gateway Together Watch Analysis Worker
...
ExecStart=$PYTHON_BIN $REPO_ROOT/scripts/run_watch_analysis_worker.py
Restart=always
RestartSec=3
KillSignal=SIGTERM
TimeoutStopSec=30
