"""
从 R2 读取指定窗口最近 12 轮原文，按 4 轮一组共 3 组，链式调用 DeepSeek 实时层总结（fetch_new_summary），
把「漏跑的三轮」补进窗口总结，与线上每满 4 轮触发一次的行为一致。

存储说明：全局总结在 R2 里仍是单个 global/summary.txt，写入时替换该对象；
但替换后的**正文**是在「当前已有总结」上逐组融合近期对话（与 pipeline 里 current + recent_4 相同），
不是从空串重写整份记忆。
...
def main() -> int:
    p = argparse.ArgumentParser(
        description="在最近 12 轮上补跑 3 次窗口总结（链式融合进当前全局总结，非清空重写）"
    )
    p.add_argument(
        "--window-id",
    ...
    p.add_argument(
        "--from-empty",
        action="store_true",
        help="调试：上文总结从空串开始（会丢掉当前 global/summary 已有内容，一般不用于补跑）",
    )
    args = p.parse_args()
    ...
    if args.from_empty:
        ...
        current = old_summary
        print(
            f"起点：当前全局总结 {len(current)} 字符；将在其上分 3 组融入最近 12 轮（与线上一轮轮融合一致）",
            flush=True,
        )
    ...
    if not ok:
        ...
        return 3
    print(
        f"已更新 global/summary.txt（单 key 覆盖写入，但内容为融合后的全文）。最终长度 {len(current)} 字符。",
        flush=True,
    )
    return 0
