...
async function loginByQrcode() {
  ...
  const r1 = await ilinkGetJson(qrUrl, "");
  if (!r1.data) {
    throw new Error(`get_bot_qrcode 失败 status=${r1.status} body=${(r1.text || "").slice(0, 200)}`);
  }
  const qrcodeToken = String(r1.data?.qrcode || "").trim();
  if (!qrcodeToken) {
    throw new Error(`get_bot_qrcode 未返回 qrcode，body=${JSON.stringify(r1.data).slice(0, 200)}`);
  }

  const scanLink = `${base}/ilink/bot/get_qrcode_status?qrcode=${encodeURIComponent(qrcodeToken)}`;

  // 有的实现会返回 qrcode_img_content（图片 base64）；这里尽量打印二维码
  // 如果拿不到图片，至少告诉你 qrcode 字符串（可用于调试/排障）
  const imgContent = String(r1.data?.qrcode_img_content || r1.data?.qrcode_img || "").trim();
  if (imgContent) {
    try {
      const buf = Buffer.from(imgContent, "base64");
      // qrcode-terminal 需要的是二维码内容字符串；这里我们不硬猜图片二维码解码，直接打印“请看终端提示”
      // 兜底：用 qrcodeToken 生成一个可扫的二维码（通常 qrcode 本身可用作二维码内容）
      void buf; // avoid unused
      qrcode.generate(qrcodeToken, { small: true });
      console.log("[wechat-ilink] 已在终端打印二维码，请用微信扫码确认绑定。");
    } catch {
      qrcode.generate(qrcodeToken, { small: true });
      console.log("[wechat-ilink] 已在终端打印二维码，请用微信扫码确认绑定。");
    }
    ...
    qrcode.generate(qrcodeToken, { small: true });
    console.log("[wechat-ilink] 未返回二维码图片字段，已用 qrcode 字符串生成终端二维码。");
  }
  ...
  while (true) {
    ...
    if (status && status !== "waiting" && status !== "scanned") {
      console.log(`[wechat-ilink] 等待扫码中 status=${status}`);
    }
  }
}
...
function isDirectChatMsg(msg) {
  // 协议里可能有 group_id/chat_type 等字段。我们尽量保守：只要有明显 group 字段就丢弃
  const gid = msg?.group_id ?? msg?.groupId ?? msg?.room_id ?? msg?.roomId;
  if (gid) return false;
  return true;
}
...
async function callGatewayChat(windowId, userText) {
  ...
  if (envBool("GATEWAY_TG_USER_INPUT", false)) {
...
async function main() {
  const enabled = envBool("WECHAT_ILINK_ENABLED", true);
  if (!enabled) {
    console.log("[wechat-ilink] WECHAT_ILINK_ENABLED=0，已退出。");
    process.exit(0);
  }
  ...
  if (!state.bot_token || forceLogin) {
    const botToken = await loginByQrcode();
    state.bot_token = botToken;
    // 首次登录游标为空即可；如你希望保留旧游标也可，但一般新登录重置更干净
    state.get_updates_buf = state.get_updates_buf || "";
    saveState(state);
  } else {
    console.log(`[wechat-ilink] 读取本地 bot_token=${maskToken(state.bot_token)}`);
  ...
  const maxReplyChars = Number(envStr("WECHAT_MAX_REPLY_CHARS", "800")) || 800;
  const dedupeMax = 2000;
  const dedupe = new Set(); // 保存最近消息 key
