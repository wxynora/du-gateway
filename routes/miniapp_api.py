...
def miniapp_sumitalk_history():
    ...
    meta = _sumitalk_request_brief()
    sumitalk_logger.info(
        "history_get_ok device_id=%s messages=%s updated_at=%s known_devices=%s file_exists=%s remote=%s ua=%s",
        device_id,
        len(messages),
...
def miniapp_sumitalk_history_save():
    ...
    if not ok:
        ...
        sumitalk_logger.error(
            "history_put_failed device_id=%s incoming=%s current=%s merged=%s remote=%s ua=%s",
            device_id,
            incoming_count,
            current_count,
            len(messages),
    ...
    meta = _sumitalk_request_brief()
    sumitalk_logger.info(
        "history_put_ok device_id=%s incoming=%s current=%s merged=%s updated_at=%s remote=%s ua=%s",
        device_id,
        incoming_count,
        current_count,
        len(messages),
...
def miniapp_sumitalk_history_migrate():
    ...
    if not ok:
        meta = _sumitalk_request_brief()
        sumitalk_logger.error(
            "history_migrate_failed old_device_id=%s new_device_id=%s old_count=%s new_count=%s merged=%s remote=%s ua=%s",
            old_device_id,
            new_device_id,
            len((old_row or {}).get("messages") or []),
            len((new_row or {}).get("messages") or []),
            len(merged_messages),
            meta["remote"],
            meta["ua"],
        )
    ...
    meta = _sumitalk_request_brief()
    sumitalk_logger.info(
        "history_migrate_ok old_device_id=%s new_device_id=%s old_count=%s new_count=%s merged=%s copied=%s updated_at=%s remote=%s ua=%s",
        old_device_id,
        new_device_id,
        len((old_row or {}).get("messages") or []),
        len((new_row or {}).get("messages") or []),
        len(merged_messages),
        bool((old_row or {}).get("messages")),
        payload["updated_at"],
        meta["remote"],
        meta["ua"],
    )
