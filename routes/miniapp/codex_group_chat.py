...
def register_routes(bp) -> None:
    ...
    def miniapp_codex_group_chat_task_create():
        ...
        if not task:
            return jsonify({"ok": False, "error": "缺少 window_id / user_message，无法创建笨笨群聊任务"}), 400
    ...
    @bp.route("/codex-group-chat-tasks/<task_id>/cancel", methods=["POST"])
    def miniapp_codex_group_chat_task_cancel(task_id: str):
        body = request.get_json(silent=True) or {}
