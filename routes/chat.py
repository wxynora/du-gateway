...
def chat_completions():
    ...
    # 注意：有些上游会把 reasoning 放在“工具中间轮次”里，最终轮次可能为空；这里累计后用于存档展示。
    ...
    for _ in range(max_tool_rounds - 1):
        ...
        try:
            if isinstance(msg, dict):
                for rk in ("reasoning", "reasoning_content", "thinking"):
                    if msg.get(rk):
                        accumulated_reasoning_parts.append(str(msg.get(rk)))
                        break
        except Exception:
            pass
                ...
                if isinstance(msg, dict) and not msg.get("reasoning"):
                    for rk in ("reasoning", "reasoning_content", "thinking"):
                        if msg.get(rk):
                            ...
                            break
                    # 最终轮次没有 reasoning 时，回填工具中间轮次累计的 reasoning
                    if not msg.get("reasoning") and accumulated_reasoning_parts:
