...
def collect_due_proactive_trigger_events(target_user_id: int = 0) -> dict:
    """Collect due hard-trigger events without sending them; async scheduler enqueues these."""
    uid = int(target_user_id or TELEGRAM_PROACTIVE_TARGET_USER_ID or 0)
    if uid <= 0:
        return {"ok": False, "error": "missing_target_user_id", "events": []}
    now_dt = _now_dt()
    if not now_dt:
        return {"ok": False, "error": "time_parse_failed", "events": []}
    window_id = f"tg_{uid}"
    doc = r2_store.get_sense_latest() or {}
    if not isinstance(doc, dict) or not doc:
        return {"ok": True, "events": [], "skip_reason": "no_sense"}
    today = now_dt.strftime("%Y-%m-%d")
    yesterday = (now_dt - timedelta(days=1)).strftime("%Y-%m-%d")
    history: list[dict] = []
    if now_dt.hour < _NIGHT_REAWAKE_END_HOUR:
    ...
    suppress_reason = _recent_normal_chat_suppress_reason(window_id, now_dt)
    if suppress_reason:
        return {"ok": True, "events": [], "skip_reason": "recent_normal_chat", "detail": suppress_reason}
    events = sorted(
        _build_events(doc, history, window_id, now_dt),
        key=lambda e: _EVENT_PRIORITY.get(e.trigger_type, 999),
    )
        ...
        out.append(
            {
                ...
                "event_at": event.event_at or now_beijing_iso(),
                "event_text": event.text(),
                "window_id": window_id,
            }
        )
...
def mark_proactive_trigger_event_fired(payload: dict, result: dict | None = None) -> bool:
    """Mark a queued hard-trigger event as fired after dispatcher attempts it."""
    if not isinstance(payload, dict):
        return False
    dedupe_key = str(payload.get("dedupe_key") or "").strip()
    trigger_type = str(payload.get("trigger_type") or "").strip()
    if not dedupe_key or not trigger_type:
        return False
    event = TriggerEvent(
    ...
    try:
        state = _read_state()
        _mark_fired(state, event, result or {})
        return True
    except Exception as e:
        logger.warning("主动硬触发标记 fired 失败 type=%s error=%s", trigger_type, e, exc_info=True)
        return False
