    ...
    return [
        ...
        },
        {
            ...
            "function": {
                ...
                "description": (
                    "通过小爱音箱执行明确控制指令。"
                    "当前稳定用途：空调（开关、模式、温度）；小爱音箱（播放音乐、切换音乐状态、音量调整）。"
                ),
                "parameters": {
                    "type": "object",
                    "properties": {
                        "command": {
                            "type": "string",
                            "description": "要让小爱执行的自然语言命令，如“打开空调”“把空调调到26度”“播放轻音乐”“停止播放”。小爱音箱自身音量请用阿拉伯数字，例如“把小爱音箱音量调到50”。",
                        },
                        "speaker_name": {
                            "type": "string",
...
def get_gateway_sex_play_draw_tools() -> List[dict]:
    """返回 sex play 抽签工具定义，供渡主动抽签/重抽/完成。"""
    return [
        {
            "type": "function",
            "function": {
...
def execute_sex_play_draw_tool(name: str, arguments: dict) -> str:
    """执行 sex play 抽签工具，返回给渡的 JSON 字符串。"""
    if name not in SEX_PLAY_DRAW_TOOL_NAMES:
    ...
    try:
        ...
        logger.exception("sex_play_draw 工具执行异常")
