...
{{
  ...
  "npc_taskers": [
    {{"name": "任务者 NPC 本名", "instance_name": "可选：副本内身份名（角色扮演副本才建议填）", "tier_note": "炮灰|新人|老练|大佬 等定位", "stance": "未知（玩家不可知；勿直给善恶）", "blurb": "一句话外貌或公开可见特征；可写其不认同副本身份"}},
...
**编制硬性规则**：`npc_taskers` 必须恰好 **4 个对象**，与两名玩家合计 **6 名任务者**；NPC 的善恶与立场对玩家默认不可知，勿在框架里直给结论。**instance_genre** 须与 `world`、`conflict` 一致；**initial_stats** 须含主神积分、双方 HP/精神、**等级与阶位（D～S）、体力与智慧、血统名称**、**双方 abilities 数组（元素为 name/desc，可无项）**、背包（可为空数组）。
...
{{
  ...
  "player2_role": "渡在本副本中的身份 + 外貌特征（简短；默认不写性格与秘密）",
  "npc_taskers": [
    {{"name": "任务者本名", "instance_name": "可选：副本内身份名（角色扮演副本才建议填）", "tier_note": "", "stance": "未知", "blurb": "外貌或公开可见特征；可写其不认同副本身份"}},
    {{"name": "", "tier_note": "", "stance": "", "blurb": ""}},
    {{"name": "", "tier_note": "", "stance": "", "blurb": ""}},
    {{"name": "", "tier_note": "", "stance": "", "blurb": ""}}
  ],
...
**编制**：`npc_taskers` 必须恰好 4 条，与两名玩家合计 6 名任务者；NPC 的立场与善恶对玩家默认不可知，框架勿直给。须带 **instance_genre**、**genre_note** 与 **initial_stats**（含等级、阶位 D～S、体力、智慧、血统、**abilities**）。
...
def _format_tasker_regiment_for_gm(fw: dict) -> str:
    ...
    lines = [
        ...
        "- 四名 NPC 可与难度相应：低难度多为炮灰、新人；高难度可出现老练者或关键「大佬」。注意：NPC 真实立场对玩家默认不可知，不得在设定里直给“好/坏”结论；禁止过度血腥虐待描写。",
        "",
        "四名 NPC 档案（须在剧情中落实）：",
    ]
