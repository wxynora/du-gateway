...
**编制硬性规则**：每个副本固定 **6 名任务者**——玩家两名（玩家一、玩家二「渡」）+ **恰好 4 名任务者 NPC**，同场竞技或同规则约束；难度 **D～S**（D 最低、S 最高），难度越高环境越险。**任务者都用自己的身体进入副本**，不更换躯体。**NPC 的善恶/立场对玩家应默认不可知**，不要在框架里直接写“好人/坏人/害人”等结论，只能给公开可见信息（外貌、身份、当下公开行为）。
...
_CANDIDATES_SYSTEM = """你在为一款「无限流」双人文字跑团生成**副本候选设定池**。
...
说明：场地为「主神空间」时表示已回到主神空间；购物、强化血统、加体力/智慧、治疗、升级与阶位变化、**能力增删改**，均须体现在面板与积分中；能力行必须与当前剧情一致（整行即完整能力列表）。
...
{{
  ...
  "reward_hint": "通关后可能获得的奖励风味一句（如积分、线索、豁免；可不写具体数字）",
    "initial_stats": {{
    "points": 100,
    "player1": {{"hp": 100, "hp_max": 100, "san": 100, "san_max": 100, "level": 1, "rank": "D", "vit": 10, "wis": 10, "bloodline": "凡人", "abilities": [{{"name": "示例被动", "desc": "一句效果"}}]}},
    "player2": {{"hp": 100, "hp_max": 100, "san": 120, "san_max": 120, "level": 1, "rank": "D", "vit": 12, "wis": 12, "bloodline": "凡人", "abilities": []}},
    "items": ["可选：与副本相关的消耗品或线索道具，无则 []"]
  }},
  "opening": "开场 4-8 句：建议含传送/白光/提示音/主神刻板广播之一；**必须**出现与玩家同场的其他任务者（四名 NPC）的登场感或存在感，再进入场景，有画面感"
}}

**编制硬性规则**：`npc_taskers` 必须恰好 **4 个对象**，与两名玩家合计 **6 名任务者**；NPC 的善恶与立场对玩家默认不可知，勿在框架里直给结论；“新人/炮灰/大佬”等仅作为系统内部定位，不可直接告诉玩家。**instance_genre** 须与 `world`、`conflict` 一致；**initial_stats** 须含主神积分、双方 HP/精神、**等级与阶位（D～S）、体力与智慧、血统名称**、**双方 abilities 数组（元素为 name/desc，可无项）**、背包（可为空数组）。
...
{{
  ...
  "initial_stats": {{
    "points": 100,
  "player1": {{"hp": 100, "hp_max": 100, "san": 100, "san_max": 100, "level": 1, "rank": "D", "vit": 10, "wis": 10, "bloodline": "凡人", "abilities": [{{"name": "", "desc": ""}}]}},
  "player2": {{"hp": 100, "hp_max": 100, "san": 120, "san_max": 120, "level": 1, "rank": "D", "vit": 12, "wis": 12, "bloodline": "凡人", "abilities": []}},
    "items": []
  }},
...
**编制**：`npc_taskers` 必须恰好 4 条，与两名玩家合计 6 名任务者；任务者使用自身身体进入副本；NPC 的立场与善恶对玩家默认不可知，且“新人/炮灰/大佬”等定位不可在叙事中直给。须带 **instance_genre**、**genre_note** 与 **initial_stats**（含等级、阶位 D～S、体力、智慧、血统、**abilities**）。
...
def _default_player_stats() -> dict:
    """文游单名玩家运行时字段默认值（等级、阶位 D～S、体力/智慧、血统、结构化能力）。"""
    return {
        "hp": 100,
        "hp_max": 100,
        "san": 100,
...
def _format_tasker_regiment_for_gm(fw: dict) -> str:
    ...
    lines = [
        ...
        "- 四名 NPC 可与难度相应（内部定位可区分新人/炮灰/老练/大佬），但这些定位不得直接告知玩家；玩家只能通过剧情表现自行判断。注意：NPC 真实立场对玩家默认不可知，不得在设定里直给“好/坏”结论；禁止过度血腥虐待描写。",
        "",
        "四名 NPC 档案（须在剧情中落实）：",
    ]
...
def _normalize_initial_stats(raw: dict) -> dict:
    ...
    def _one(pk: str) -> dict:
        ...
        default_hp_max = 100
        default_san_max = 120 if pk == "player2" else 100
        default_vit = 12 if pk == "player2" else 10
        default_wis = 12 if pk == "player2" else 10
        hm = max(1, int(d.get("hp_max") or default_hp_max))
        sm = max(1, int(d.get("san_max") or default_san_max))
        ...
        vit = max(0, int(d.get("vit") or d.get("vitality") or default_vit))
        wis = max(0, int(d.get("wis") or d.get("wisdom") or default_wis))
...
def _format_status_footer(session: dict) -> str:
    ...
    def _foot_player(p: dict) -> str:
        return (
            ...
            f"Lv{p.get('level', 1)}·{p.get('rank', 'D')}阶｜体{p.get('vit', 0)} 智{p.get('wis', 0)}｜{p.get('bloodline', '凡人')}｜{_foot_abilities(p)}"
        )
...
def format_candidate_expansion_prompt(candidate: Any) -> str:
    ...
    return (
        ...
        "必须保留候选的核心题材、危险钩子与悬念，但可以补全 NPC、规则、任务、开场和主神面板。\n\n"
        f"副本名：{item['title']}\n"
...
def _player_summary_for_card(player: Any) -> str:
    ...
    return (
        f"HP {p.get('hp', 0)}/{p.get('hp_max', 0)}，精神 {p.get('san', 0)}/{p.get('san_max', 0)}，"
        f"Lv{p.get('level', 1)}·{p.get('rank', 'D')}阶，体{p.get('vit', 0)}/智{p.get('wis', 0)}，"
        f"血统：{p.get('bloodline', '凡人')}，能力：{ab_names}"
    )
...
def _current_instance_for_card(session: dict) -> dict:
    ...
    inventory = [_compact_text(x, 40) for x in (st.get("inventory") or []) if str(x).strip()][:20]
    return {
        ...
        "phase": "主神空间" if st.get("phase") == "hub" else "副本",
...
def get_session_view(user_id: int) -> dict:
    ...
    return {
        ...
        "session": {
            ...
            "phase": str(session.get("phase") or st.get("phase") or "instance"),
            "framework": {
...
def cmd_use_item(user_id: int, item_name: str, action: str = "") -> str:
    """使用道具：先校验背包，再作为一次行动交给 GM 判定是否消耗/生效。"""
    uid = int(user_id)
    ...
    _session_ensure_stats(session)
    inventory = [str(x).strip() for x in (session.get("stats") or {}).get("inventory") or [] if str(x).strip()]
    if item not in inventory:
        return f"文游：背包里没有【{item}】。"
    detail = str(action or "").strip()
    text = f"使用道具【{item}】" + (f"：{detail}" if detail else "。")
    return cmd_action(uid, text, "player1")
...
def cmd_use_item_with_du(user_id: int, item_name: str, action: str = "") -> tuple[str, str]:
    ...
    detail = str(action or "").strip()
    text = f"使用道具【{item}】" + (f"：{detail}" if detail else "。")
    return cmd_action_with_du(uid, text)
...
def cmd_end(user_id: int) -> str:
    ...
    session.setdefault("history", []).append(
        {
            "role": "gm",
            "content": "【主神提示】副本已结束，进入系统空间结算阶段。可进行领取奖励、购买道具、强化血统与整备；整备完成后请完成最终结算归档。",
            "timestamp": ts,
        }
    )
    r2_store.save_wenyou_session(uid, session)
    return "文游：已进入系统空间结算阶段。现在可进行奖励与整备操作；结算完成后请完成最终结算归档本局。"
