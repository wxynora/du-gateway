...
{{
  ...
  "reward_hint": "通关后可能获得的奖励风味一句（如积分、线索、豁免；可不写具体数字）",
    ...
    "player2": {{"hp": 100, "hp_max": 100, "san": 100, "san_max": 100, "level": 1, "rank": "D", "vit": 10, "wis": 10, "bloodline": "凡人", "abilities": []}},
...
def _format_tasker_regiment_for_gm(fw: dict) -> str:
    ...
    for i, n in enumerate(fw.get("npc_taskers") or []):
        if isinstance(n, dict):
            lines.append(
                f"  · {i+1}. 「{n.get('name', '')}」｜{n.get('tier_note', '')}｜公开信息：{n.get('blurb', '')}（立场：{n.get('stance', '未知')}）"
            )
    return "\n".join(lines)
