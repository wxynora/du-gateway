...
**编制硬性规则**：`npc_taskers` 必须恰好 **4 个对象**，与两名玩家合计 **6 名任务者**；NPC 可有好人、坏人、坑货、炮灰、大佬，与 `difficulty` 相匹配。**instance_genre** 须与 `world`、`conflict` 一致；**initial_stats** 须含主神积分、双方 HP/精神、**等级与阶位（D～S）、体力与智慧、血统名称**、**双方 abilities 数组（元素为 name/desc，可无项）**、背包（可为空数组）。
...
**编制**：`npc_taskers` 必须恰好 4 条，与两名玩家合计 6 名任务者；NPC 可炮灰可大佬、可善可恶。须带 **instance_genre**、**genre_note** 与 **initial_stats**（含等级、阶位 D～S、体力、智慧、血统、**abilities**）。
