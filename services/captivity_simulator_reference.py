...
def get_reference(category: str) -> str:
    ...
    if category == "物品":
        return "\n".join([
            "可赠送物品：" + _labels(INVENTORY_ITEMS) + "。赠送和收回不占白天行动，也不会单独触发同步；任意时段送出的礼物都会加入待发放队列，入夜后统一发放。待发放或已赠送的物品不能重复赠送。",
            "礼物可带附言；入夜发放时，对方会看到礼物名称和附言，可交互物品会同时加入当晚可选行动。",
            ...
            "提交示例：【赠送物品：书 书名=夜航船 彩蛋='逐行填写' 附言='今晚再拆'】",
        ])
    if category == "夜间":
        lines = []
