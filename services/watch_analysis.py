...
ANALYSIS_SYSTEM_PROMPT = "\n".join(
    [
        ...
        "识别出作品不等于确认当前人物身份。作品知识只能辅助理解，不能替代当前样本证据。凡是未在 previous_adjacent_plot_chunks 中出现的新配角，除非当前字幕或补充文字直接给出姓名或关系，description 和 characters 都必须先使用可见特征或泛称；仅凭长相熟悉不得命名，即使你非常熟悉作品也一样。对白中的称呼与候选关系冲突时，以当前对白为准；例如主角称对方‘阿姨’或‘叔叔’时，不得擅自写成主角的妈妈或爸爸。",
        "只分析给出的样本，不补写未采样画面，不把作品知识当成当前片源时间点已经发生的证据。稀疏样本之间不得擅自补全动作过程。",
        "plot_chunks.description 是唯一的主剧情文字。只按真实剧情单元切分，不按样本数量切分；同一条连续的任务、行动或道具作用即使跨越很多样本，也应保持在同一条剧情线上。遇到片头、标题卡、时间地点跳转或新故事开始时再明确分段，并自然交代转换。previous_adjacent_plot_chunks 只用于接住批次边界，不要在输出中复述上一批剧情。",
        "story_background 是否产出由本次 system 消息末尾的【剧情背景输出模式】决定，必须严格遵守，不能自行改换模式。它不是累计剧情，也不承担跨批次状态保存。",
        ...
        "输出前静默核对：每个剧情段是否说清了‘发生了什么以及局面如何变化’；跨镜头重复出现的任务、道具和结果是否已经合成同一条剧情主线；留下的每个画面细节是否都在帮助理解剧情；新配角是否遵守 previous_adjacent_plot_chunks 的证据边界；称呼类台词是否保留原称呼；主剧情是否完全没有‘字幕显示’或‘画面显示’。发现违反时先修正再输出。",
...
KNOWN_BACKGROUND_SYSTEM_PROMPT = "\n".join(
    [
        "【剧情背景输出模式：不产出】",
        "本会话中陪伴者已经了解这部作品。story_background.background 必须输出空字符串，characters 必须输出空数组，不要生成给陪伴者阅读的剧情背景。",
    ]
)
...
NEEDS_SUMMARY_BACKGROUND_SYSTEM_PROMPT = "\n".join(
    [
        "【剧情背景输出模式：产出】",
        "本会话中陪伴者需要剧情背景。purpose=rolling 时，story_background.background 只说明理解本批剧情所需、截至 through_ms 已经揭示的人物、关系、故事前提和必要专有名词；不要续写累计剧情，也不要复述整部作品。",
        "即使输入带有完整作品知识卡，也绝不能写入 through_ms 之后才揭示的身份、关系或事件。purpose=identify 或 timeline_prepass 时，story_background.background 必须输出空字符串，characters 必须输出空数组。",
    ]
)
...
ANALYSIS_SCHEMA = {
    ...
    "properties": {
        ...
        "story_background": {
            ...
            "required": ["through_ms", "background", "characters"],
            "additionalProperties": False,
        },
        "risk_events": {
            "type": "array",
...
def _adjacent_plot_context(session: dict, job: dict) -> list[dict]:
    if str(job.get("purpose") or "") != "rolling":
        return []
    session_id = str(session.get("session_id") or "").strip()
    if not session_id:
        return []
    range_start_ms = _int(job.get("range_start_ms"), 0)
    range_end_ms = _int(job.get("range_end_ms"), range_start_ms)
    previous_window_start_ms = max(0, range_start_ms - max(1, range_end_ms - range_start_ms))
    timeline_epoch = _int((session.get("playback") or {}).get("timeline_epoch"), 0)
    chunks = watch_runtime_store.get_completed_plot_chunks(
        session_id,
        timeline_epoch=timeline_epoch,
        through_ms=range_start_ms,
    )
    return [
        {
        ...
        for item in chunks
        if _int(item.get("end_ms"), 0) >= previous_window_start_ms
    ]
...
def build_watch_analysis_prompt(session: dict, job: dict, samples: list[dict]) -> str:
    ...
    context = {
        ...
        "previous_adjacent_plot_chunks": _adjacent_plot_context(session, job),
    ...
    return "\n".join(
        [
            "请分析下面这一批样本。previous_adjacent_plot_chunks 仅用于衔接上一批结尾，当前批次发生了什么仍以本批音频、画面和字幕为准。",
...
def normalize_watch_analysis_result(raw: dict, *, session: dict, job: dict, samples: list[dict]) -> dict:
    ...
    background_raw = raw.get("story_background") if isinstance(raw.get("story_background"), dict) else {}
    through_ms = min(sample_max, _int(background_raw.get("through_ms"), sample_max))
    story_background = {
        "through_ms": through_ms,
        "background": (
            _text(background_raw.get("background"), 5000)
            if _knowledge_mode(session) == "needs_summary"
            else ""
        ),
        "characters": (
            _strings(background_raw.get("characters"), limit=40)
            if _knowledge_mode(session) == "needs_summary"
            else []
        ),
    }
