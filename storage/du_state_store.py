...
def get_du_longterm_memory() -> Optional[dict]:
    client = _s3_client()
    if not client:
        return None
    data = _read_json(client, R2_KEY_DU_LONGTERM_MEMORY)
    if not isinstance(data, dict):
        return None
    return data


def save_du_longterm_memory(data: dict) -> bool:
    client = _s3_client()
    if not client:
        return False
    if not isinstance(data, dict):
    ...
    with _du_state_write_lock:
        try:
            _write_json(client, R2_KEY_DU_LONGTERM_MEMORY, data)
            return True
        except Exception as e:
