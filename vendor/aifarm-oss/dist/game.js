...
import { advance, plant, water, harvest, upgradeLand, shopOffer, collectionPct, codexCountByCategory, nextUpgradeReq, buyItem, useItem, craft, designCrop, plantBatch, waterAll, harvestAll, usePotionBatch, refreshShop, buyRecipe, affordablePotions, buyPotionSet, potionDailyLeft, shopAnimals, nextLockedAnimal, buyAnimalForPartner, ranchRoamLine, decorLines, takeInbox, shopPets, nextLockedPet, buyPetForPartner, potionTargets, circledNum, } from "./engine.js";
...
export function viewMarket(f, own, viewer) {
...
export function visitView(f, now, viewer) {
    ...
    if (growing)
        lines.push(`💧 有 ${growing} 块作物在长，帮 TA 浇水给最快熟的那块加速 30min（默认浇剩余时间最短的），还常掉 1 瓶加速药水（每家每天 1 次，你每天上限 10 瓶）　→ water {"to":"${f.id}"}`);
    if (f.shop.potionSet)
        lines.push(`🎁 这家店刷出了药水套装（${f.shop.potionSet.qty} 瓶加速药水 ${f.shop.potionSet.price} 金，限购 1）　→ buy-potion-set {"to":"${f.id}"}`);
