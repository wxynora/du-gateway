...
import { titles, cropById, type TitleDef } from "./content.js";
...
export function metricValue(f: Farm, field: string): number {
  switch (field) {
...
export const titleById = (id: string): TitleDef | undefined => titles.find((t) => t.id === id);

/** 已解锁（达到阈值）？ */
export const isUnlocked = (f: Farm, id: string): boolean => {
 ...
 */
export function checkTitles(f: Farm): TitleDef[] {
  f.titles ??= [];
  const fresh: TitleDef[] = [];
  for (const t of titles) {
    if (f.titles.includes(t.id)) continue;
 ...
 */
export function concordTierName(n: number): string {
  const v = Math.min(100, Math.max(0, n ?? 0));
  const bands = titles.filter((t) => t.field === "expConcord").sort((a, b) => b.min - a.min);
  for (const t of bands) if (v >= t.min) return t.name;
  return "尚需磨合";
}

/** 当前佩戴的称号（必须仍在已解锁列表里，否则视为未佩戴）。 */
export function equippedTitle(f: Farm): TitleDef | undefined {
  if (!f.titleEquipped) return undefined;
  if (!(f.titles ?? []).includes(f.titleEquipped)) return undefined;
  return titleById(f.titleEquipped);
}

/** 名字前缀：佩戴了称号就返回「✧称号✧」，否则空串。 */
export function titlePrefix(f: Farm): string {
  const t = equippedTitle(f);
  return t ? `✧${t.name}✧` : "";
}

/** 农场名（含佩戴的称号前缀）——排行榜/串门页统一用它。 */
export const nameWithTitle = (f: Farm): string => `${titlePrefix(f)}${f.name}`;

/**
 * 佩戴 / 卸下称号。id 为空串=卸下；id 必须是已解锁的称号。返回结果文案。
 * 注意：只改 titleEquipped，不发奖、不解锁。
 */
export function equipTitle(f: Farm, id: string): { ok: boolean; text: string } {
  checkTitles(f); // 先把刚达标的补登，免得"明明够了却选不了"
  if (!id) { f.titleEquipped = undefined; return { ok: true, text: "已卸下称号。" }; }
  const t = titleById(id);
  if (!t) return { ok: false, text: "没有这个称号。" };
  if (!(f.titles ?? []).includes(id)) return { ok: false, text: `还没解锁「${t.name}」，达成条件才能佩戴。` };
  f.titleEquipped = id;
