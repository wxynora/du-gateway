...
const FARM_TOOL = {
  name: "farm",
  description:
    "在 AI 农场里执行一个动作（种地/浇水/收获/熔炼/串门/留言…）。一个工具走天下：动作名放 action，其余参数平铺在同级，"
...
export interface McpCtx {
  serverName: string;
  // 把一次工具调用翻成引擎结果。由 server.ts 提供（闭包捕获 runFarm + 当前农场身份）。
  run: (action: string, params: Record<string, unknown>) => { ok: boolean; text: string };
}

// 处理单条 JSON-RPC 请求。返回响应对象；通知（无 id 的 method）返回 undefined 表示「无需回应」。
function handleOne(rpc: any, ctx: McpCtx): any | undefined {
  ...
  switch (rpc?.method) {
    ...
    case "tools/call": {
      if (rpc?.params?.name !== "farm") return fail(-32602, `未知工具：${rpc?.params?.name}。这个服务只有一个工具 farm。`);
    ...
    default:
      if (typeof rpc?.method === "string" && rpc.method.startsWith("notifications/")) return undefined; // 通知一律不回
      return fail(-32601, `不支持的方法：${rpc?.method}`);
  }
}

// 入口：支持单条与 JSON-RPC 批量（数组）。返回 undefined 表示全是通知、HTTP 应回 202 空体。
export function mcpDispatch(rpc: any, ctx: McpCtx): any | undefined {
