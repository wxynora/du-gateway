...
import { flavor, landTierByLevel, getCrop, totalCropCount, type Crop, type Quality } from "./content.js";
...
export function statusFooter(farm: Farm, now: number): string {
  let ripe = 0, grow = 0, empty = 0;
  for (const p of farm.plots) {
    if (!p.crop) empty++;
    else if (p.crop.ripe) ripe++;
    else grow++;
  }
...
export function plantText(limitedIds: string[] = []): string {
  if (limitedIds.length)
    return limitedIds
      .map((id) => {
        const c = getCrop(id);
        if (c?.plantLine) return c.plantLine; // 专属下种台词优先
...
export function waterText(isOwner: boolean, by: string): string {
...
export function harvestText(crop: Crop, quality: Quality, value: number, isNew: boolean, codexReward = 0, byDesigner = false): string {
  const tag = isNew ? "  ✨新图鉴" : "";
...
// —— 收获奖励事件 ——
export function bonusEventText(bonus: { text: string; effectType: string; extraCoins: number; ripened: number } | null): string {
  if (!bonus) return "";
  let s = bonus.text;
  if (bonus.effectType === "额外金币" && bonus.extraCoins) s += `（+${bonus.extraCoins} 金）`;
  if (bonus.effectType === "连收" && bonus.ripened) s += `（相邻 ${bonus.ripened} 块地也熟了）`;
  return s;
}
// —— 素材掉落（教学语「攒齐可熔炼」改为每轮一句汇总，见 game.composeHarvests）——
export function dropText(drop: { name: string; rarity: string; desc: string } | null): string {
  if (!drop) return "";
  return `🪨 掉落素材【${drop.name}·${drop.rarity}】——${drop.desc}`;
}
export function potionDropText(): string {
  return "🧪 运气不错，土里还埋着一瓶加速药水，顺手收进了背包！";
}

// —— 偷菜 ——
export function stealThiefText(crop: Crop): string {
  const line = flavor.steal.thiefByRarity[crop.rarity] ?? "得手了，赶紧溜。";
  return `${line}\n（偷到 ${crop.name}）`;
}
export function stealVictimLog(crop: Crop, by: string): string {
  return pick(flavor.steal.victim).replaceAll("{by}", by);
}

// —— 巡视农场 ——
export function describeFarm(farm: Farm, now: number, opts: { visitor?: boolean } = {}): string {
  const lines: string[] = [];
  const season = currentSeason(now);

  // 原创(ugc)作物受保护、不能偷——串门时单独归类；ripe 仍含 ugc（主人自己能收）。
  const isUgc = (c: NonNullable<typeof farm.plots[number]["crop"]>) =>
    c.seedType === "limited" && !!c.limitedId && getCrop(c.limitedId)?.category === "ugc";
  let ripe = 0, growing = 0, empty = 0, protectedUgc = 0;
  for (const p of farm.plots) {
    if (!p.crop) empty++;
    else if (p.crop.ripe) { ripe++; if (isUgc(p.crop)) protectedUgc++; }
    else growing++;
  }
  if (opts.visitor) {
    ...
    if (stealable) lines.push(`有 ${stealable} 块地结着成熟的神秘作物，可以偷（steal），也可以帮浇水（water）。`);
    if (protectedUgc) lines.push(`还有 ${protectedUgc} 块结着原创作物，受保护偷不了——想要就去集市买它的种子自己种。`);
    if (growing) lines.push(`${growing} 块地还在长，看不出是什么。`);
    if (!stealable && !growing && !protectedUgc) lines.push("地里暂时没什么可下手的，空荡荡的。");
    return lines.join("\n");
  }

  lines.push(`🌾 你站在「${farm.name}」的地头。【${season.name} · ${landTierByLevel(farm.landTier).name}】`);
  lines.push(pick(season.ambience));
  if (ripe >= 3) lines.push(pick(flavor.ambient.bumper));
  else if (ripe) lines.push(pick(flavor.ambient.ripe));
  if (growing) lines.push(`${growing} 块地里的神秘幼苗正在长，收获才知是什么。`);
  if (empty && empty === farm.plots.length) lines.push(pick(flavor.ambient.allEmpty));
  else if (empty) lines.push(pick(flavor.ambient.empty));
  return lines.join("\n");
}
