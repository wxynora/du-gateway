...
import type { Farm, TaskSlot } from "./types.js";

/** 任务进度事件：动作发生处触发，匹配「进行中」任务则推进。 */
export type TaskEventKind =
  | "harvest"      // 收获一株作物（data: {rarity, isNew, isUgc}）
  | "craft"        // 熔炼一次
  | "plant_ugc"    // 种下一株自创作物
  | "help_water"   // 帮邻居浇一次水（计在浇水者头上）
  | "got_watered"  // 被人浇了一次水（计在被浇者头上）
  | "message"      // 给邻居留一次言
  | "visit"        // 串门一家农场（data: {targetId}，按家去重）
  | "steal"        // 偷一次菜得手
  | "got_stolen"   // 被偷一次菜（计在被偷者头上）
  | "buy_ugc";     // 买一次邻居的原创作物
...
interface TaskDef {
  ...
  /** 这座农场当前能不能刷到这条（避免死任务）*/
  eligible: (f: Farm) => boolean;
}

// 社交类任务：只有「访问」开着（没闭门谢客）才会刷到
const social = (f: Farm) => f.social?.visit !== false;
// 有自创种子在背包（自创作物 id 以 "ugc_" 开头，见 engine.designCrop），才刷「种自创」
const hasUgcSeed = (f: Farm) => Object.entries(f.seeds ?? {}).some(([id, n]) => (n || 0) > 0 && id.startsWith("ugc_"));
...
const defByKind = (kind: string) => TASK_POOL.find((t) => t.kind === kind);

/** 确定性 0..1（farmId+seq）：roll offer 用，不消耗 rngState（同 farm 同 seq 恒定）。 */
function hash01(id: string, seq: number): number {
  let h = (2166136261 ^ seq) >>> 0;
  for (let i = 0; i < id.length; i++) { h ^= id.charCodeAt(i); h = Math.imul(h, 16777619); }
  return ((h >>> 0) % 100000) / 100000;
}

const rewardText = (t: { reward: number; currency: string }) => t.currency === "coin" ? `+${t.reward} 金` : `+${t.reward} 银`;
function describe(slot: TaskSlot): string {
  return (defByKind(slot.kind)?.label ?? slot.kind).replace("{n}", String(slot.target));
}

/** 当天已接取数（顺带做每日重置）。 */
function dailyTaken(f: Farm, now: number): number {
  const day = currentDayIndex(now);
  if (!f.taskDaily || f.taskDaily.day !== day) f.taskDaily = { day, taken: 0 };
  return f.taskDaily.taken;
}

/** 按权重 + 资格随机出一条新任务（确定性，靠 seq）。 */
function rollOffer(f: Farm, seq: number, now: number): TaskSlot {
  const pool = TASK_POOL.filter((t) => t.eligible(f));
  const total = pool.reduce((s, t) => s + t.weight, 0);
  let r = hash01(f.id, seq) * total;
  let chosen = pool[pool.length - 1];
  for (const t of pool) { if (r < t.weight) { chosen = t; break; } r -= t.weight; }
...
export function tickTask(f: Farm, now: number): void {
  if (f.id === NPC_ID) return;
  const taken = dailyTaken(f, now);
  if (taken >= TASK_DAILY_CAP) { if (f.task && !f.task.accepted) f.task = undefined; return; } // 今日已满：撤掉未接取的 offer
  const t = f.task;
  if (!t) { f.task = rollOffer(f, 1, now); return; }
  if (t.accepted && !t.completedAt) return;                         // 进行中：保留直到完成
  if (t.completedAt) {                                              // 已完成：冷却到点刷下一条
    if (now - t.completedAt >= TASK_COOLDOWN_MS) f.task = rollOffer(f, t.seq + 1, now);
    return;
  }
  if (now - t.offeredAt >= TASK_OFFER_TTL_MS) f.task = rollOffer(f, t.seq + 1, now); // 未接取超时：换一条
}

/** 接取当前 offer（消耗一个每日额度）。 */
export function acceptTask(f: Farm, now: number): { ok: boolean; text: string } {
  if (f.id === NPC_ID) return { ok: false, text: "NPC 不接任务。" };
  tickTask(f, now);
  const t = f.task;
  if (!t || t.accepted || t.completedAt) return { ok: false, text: t?.accepted ? "任务已经接取了，完成它吧。" : "现在没有可接取的任务（冷却中或今日已满）。" };
  const taken = dailyTaken(f, now);
  if (taken >= TASK_DAILY_CAP) return { ok: false, text: `今天的任务额度已用完（${TASK_DAILY_CAP}/${TASK_DAILY_CAP}），明天再来。` };
  t.accepted = true;
  t.offeredAt = now;
...
export function onTaskEvent(f: Farm, event: TaskEventKind, now: number, data: any = {}, count = 1): boolean {
  if (f.id === NPC_ID) return false;
  const t = f.task;
  if (!t || !t.accepted || t.completedAt) return false;
  const def = defByKind(t.kind);
  if (!def || def.event !== event) return false;
  if (def.match && !def.match(data)) return false;
  if (event === "visit") { // 串门按家去重：同一家只算一次
    const id = String(data.targetId ?? "");
    if (!id) return false;
    t.visited ??= [];
    if (t.visited.includes(id)) return false;
    t.visited.push(id);
  }
  t.progress = Math.min(t.target, t.progress + count);
  if (t.progress >= t.target) {
    if (t.currency === "coin") f.coins += t.reward; else f.silver += t.reward;
    t.completedAt = now;
...
export function taskLine(f: Farm, now: number): string {
  if (f.id === NPC_ID) return "";
  tickTask(f, now);
  const taken = dailyTaken(f, now);
  const t = f.task;
  if (!t) return taken >= TASK_DAILY_CAP ? `🎯 今日任务已满 ${TASK_DAILY_CAP}/${TASK_DAILY_CAP}，明天再来` : "";
  if (t.completedAt) {
    ...
    if (!t.completedShown) { t.completedShown = true; return `✅ 任务完成：${describe(t)} ${rewardText(t)}！${tail}`; }
    return `⏳ ${tail}`;
  }
  if (t.accepted) return `🎯 任务：${describe(t)} ${t.progress}/${t.target} → ${rewardText(t)}`;
  return `🎯 新任务可接：${describe(t)} → ${rewardText(t)}（accept-task 接取后才计数 · 今日 ${taken}/${TASK_DAILY_CAP}）`;
}

/** 当前是否有「可接取」的 offer（给 Agent 页决定要不要出「接取」按钮）。 */
export function hasOpenOffer(f: Farm, now: number): boolean {
  tickTask(f, now);
  const t = f.task;
  return !!t && !t.accepted && !t.completedAt && dailyTaken(f, now) < TASK_DAILY_CAP;
}
/** 给「接取」按钮用的一句话摘要。 */
export function offerSummary(f: Farm): string {
  return f.task ? `${describe(f.task)}（奖 ${rewardText(f.task)}）` : "";
}

/** 结构化任务视图（farmView/人类前端用；无任务返回 null）。 */
export function taskView(f: Farm, now: number): {
  kind: string; desc: string; target: number; progress: number; reward: number; currency: string; accepted: boolean; completed: boolean;
} | null {
  const t = f.task;
  if (!t) return null;
