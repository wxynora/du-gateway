...
export function uiRanch(f: Farm, now: number, key: string, flash?: string): string {
  ...
  for (const a of list) { const k = animalById.get(a.kindId); if (k) pendingValue += Math.round(a.pending * k.producePrice * (1 + ((a.level ?? 1) - 1) * RANCH_LEVEL_INCOME_STEP)); }
...
function lbRow(rank: number, name: string, value: number, unit: string, max: number, isMe: boolean, by?: string, off?: boolean, title?: string, code?: string, byCode?: string): string {
...
export function uiLeaderboard(f: Farm, now: number, key: string): string {
  ...
  const today = currentDayIndex(now);
  type Def = { icon: string; title: string; unit: string; sub?: string; rows: BoardRow[]; score: (x: Farm) => number };
  const defs: Def[] = [
  ...
  const mkCard = (d: Def) => {
    ...
    let foot = "";
    if (meVal > 0 && !inRows) foot = lbRow(meRank - 1, f.name, meVal, d.unit, max, true, undefined, true, equippedTitle(f)?.name, f.id);
