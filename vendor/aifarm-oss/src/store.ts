...
export function createFarm(name: string, opts?: { aiName?: string; humanName?: string }): Farm {
  const farm = makeFarm(name, undefined, opts);
  while (farms.has(farm.id)) farm.id = genCode(); // 门牌号撞号兜底（6 位空间大，几乎不会触发）
  farms.set(farm.id, farm);
  save();
  return farm;
}

export const getFarm = (id: string) => farms.get(id);
...
function ensureNpc(): boolean {
  if (farms.has(NPC_ID)) return false;
...
export function load(): void {
  if (existsSync(UGC_FILE)) {
    try { loadUgc(JSON.parse(readFileSync(UGC_FILE, "utf8"))); } catch { /* 忽略 */ }
  }
  if (!existsSync(DATA_FILE)) { ensureNpc(); save(); return; } // 全新启动：先把常驻 NPC 阿土建出来
  try {
    const arr: Farm[] = JSON.parse(readFileSync(DATA_FILE, "utf8"));
    farms.clear();
    for (const f of arr) {
      f.materials ??= {}; // 向后兼容：老存档补默认
      f.seeds ??= {};
      f.shop ??= { refreshAt: 0, recipe: null, potionSet: null };
      if (f.shop.potionSet === undefined) f.shop.potionSet = null; // 老存档补药水套装字段
      f.knownRecipes ??= [];
      f.market ??= [];
      f.silver ??= 0;
      f.codex ??= {};
    ...
    try { renameSync(DATA_FILE, bak); } catch {}
    console.error(`[store] 存档损坏，已备份到 ${bak}，以空状态启动:`, err);
  }
  if (ensureNpc()) save(); // 老存档没有阿土时补建并落盘
}
