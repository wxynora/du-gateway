...
def build_player_view(
    *,
    ...
    lines = [
        ...
        "系统波次: " + _wave_progress_line(wave_progress),
    ]
...
def _event_lines(events: list[dict[str, Any]], *, limit: int = 12) -> list[str]:
...
def _event_line(event: dict[str, Any]) -> str | None:
    ...
    if event_type == "boss_event_action":
        ...
        if action == "summon_zombie":
            return f"{event.get('boss_id', 'boss')} 召唤: {lane}路{ZOMBIE_NAMES.get(str(event.get('zombie_type')), event.get('zombie_type'))}"
        if action == "smash_cell":
            return f"{event.get('boss_id', 'boss')} 砸格: {lane}路{col}列"
