...
def cmd(text: str, save_path: str | Path | None = None) -> str:
    ...
    if not parts:
        return _current_view(session, path)

    outputs: list[str] = []
    for part in parts[:12]:
        outputs.append(_route_command(session, part, save_path=path))

    _save_session(session, path)
    return "\n\n".join(output for output in outputs if output).strip()


def _route_command(session: dict[str, Any], part: str, *, save_path: Path) -> str:
    words = shlex.split(part)
    if not words:
        return _current_view(session, save_path)
