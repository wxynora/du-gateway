...
DEFAULT_RECORDS_PATH = Path(os.environ.get("DU_IMITATOR_PVZ_RECORDS", Path.cwd() / "du_imitator_pvz_records.json"))
...
def cmd(text: str, save_path: str | Path | None = None, records_path: str | Path | None = None) -> str:
    path = Path(save_path) if save_path is not None else DEFAULT_SAVE_PATH
    record_path = _records_path_for(save_path=save_path, records_path=records_path, save_file=path)
    session = _load_or_create_session(path)
    ...
    if not parts:
        output = _current_view(session, path, records)
        _save_records(records, record_path)
        return output

    outputs: list[str] = []
    for part in parts[:12]:
        output = _route_command(session, part, save_path=path, records=records)
...
def _route_command(session: dict[str, Any], part: str, *, save_path: Path, records: dict[str, Any]) -> str:
    words = shlex.split(part)
    if not words:
        return _current_view(session, save_path, records)
...
def _current_view(session: dict[str, Any], save_path: Path | None = None, records: dict[str, Any] | None = None) -> str:
    records = records if records is not None else _load_records(_records_path_for(save_path=save_path, records_path=None, save_file=save_path or DEFAULT_SAVE_PATH))
    if _session_game_over(session):
        text = _reset_finished_session(session, records)
...
def _records_path_for(
    *,
    ...
    if save_path is not None:
        return save_file.with_name(f"{save_file.stem}_records.json")
...
def _save_records(records: dict[str, Any], records_path: Path) -> None:
...
def _engine_from_json(payload: dict[str, Any]) -> GameEngine:
    ...
    engine = GameEngine(
        config=config,
        seed=payload.get("rng", {}).get("seed", "DU-P2-001"),
