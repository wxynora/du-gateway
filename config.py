...
MUSIC_AUDIO_MAX_BYTES = int(os.environ.get("MUSIC_AUDIO_MAX_BYTES", str(80 * 1024 * 1024)))
MUSIC_AUDIO_TTL_SECONDS = int(os.environ.get("MUSIC_AUDIO_TTL_SECONDS", "7200"))


# 一起看视觉分析：独立于主聊天，不写 R2，不沿用主聊天模型选择。
WATCH_ANALYSIS_ENABLED = os.environ.get("WATCH_ANALYSIS_ENABLED", "1").strip().lower() in ("1", "true", "yes")
WATCH_ANALYSIS_API_URL = os.environ.get(
...
WATCH_ANALYSIS_MAX_FRAMES_PER_JOB = _env_int("WATCH_ANALYSIS_MAX_FRAMES_PER_JOB", 8, 1, 24)
WATCH_ANALYSIS_MAX_SAMPLE_BYTES = _env_int("WATCH_ANALYSIS_MAX_SAMPLE_BYTES", 1_500_000, 100_000, 8_000_000)
WATCH_ANALYSIS_MAX_AUDIO_BYTES = _env_int("WATCH_ANALYSIS_MAX_AUDIO_BYTES", 1_500_000, 100_000, 8_000_000)
WATCH_ANALYSIS_MAX_REQUEST_BYTES = _env_int("WATCH_ANALYSIS_MAX_REQUEST_BYTES", 10_000_000, 500_000, 40_000_000)
WATCH_ANALYSIS_MAX_LONG_EDGE = _env_int("WATCH_ANALYSIS_MAX_LONG_EDGE", 960, 320, 1568)
WATCH_ANALYSIS_SAMPLE_TTL_SECONDS = _env_int("WATCH_ANALYSIS_SAMPLE_TTL_SECONDS", 7200, 300, 86400)
WATCH_ANALYSIS_CLIENT_PLAN_TTL_SECONDS = _env_int(
    "WATCH_ANALYSIS_CLIENT_PLAN_TTL_SECONDS", 900, 60, 3600
)
WATCH_CLIENT_LEASE_SECONDS = _env_int("WATCH_CLIENT_LEASE_SECONDS", 90, 30, 600)
WATCH_ANALYSIS_JOB_MAX_ATTEMPTS = _env_int("WATCH_ANALYSIS_JOB_MAX_ATTEMPTS", 3, 1, 8)
WATCH_ANALYSIS_JOB_STALE_SECONDS = _env_int("WATCH_ANALYSIS_JOB_STALE_SECONDS", 300, 30, 1800)
WATCH_ANALYSIS_WORKER_IDLE_SECONDS = _env_float("WATCH_ANALYSIS_WORKER_IDLE_SECONDS", 0.5, 0.1, 30.0)
WATCH_ANALYSIS_FORWARD_WINDOW_MS = _env_int(
    "WATCH_ANALYSIS_FORWARD_WINDOW_MS", 1_800_000, 60_000, 1_800_000
)
WATCH_ANALYSIS_INITIAL_READY_BUFFER_MS = _env_int(
    "WATCH_ANALYSIS_INITIAL_READY_BUFFER_MS", 300_000, 60_000, 900_000
)
WATCH_ANALYSIS_FEAR_READY_BUFFER_MS = _env_int("WATCH_ANALYSIS_FEAR_READY_BUFFER_MS", 120_000, 30_000, 300_000)
WATCH_ANALYSIS_SOURCE_ENABLED = os.environ.get("WATCH_ANALYSIS_SOURCE_ENABLED", "1").strip().lower() in ("1", "true", "yes")
WATCH_ANALYSIS_SOURCE_PROVIDER = os.environ.get("WATCH_ANALYSIS_SOURCE_PROVIDER", "bilibili_api").strip() or "bilibili_api"
WATCH_ANALYSIS_SOURCE_TIMEOUT_SECONDS = _env_int("WATCH_ANALYSIS_SOURCE_TIMEOUT_SECONDS", 90, 15, 300)
WATCH_ANALYSIS_SOURCE_CACHE_SECONDS = _env_int("WATCH_ANALYSIS_SOURCE_CACHE_SECONDS", 600, 30, 3600)
WATCH_ANALYSIS_SOURCE_MAX_HEIGHT = _env_int("WATCH_ANALYSIS_SOURCE_MAX_HEIGHT", 540, 240, 1080)
WATCH_ANALYSIS_SOURCE_MAX_WORKERS = _env_int("WATCH_ANALYSIS_SOURCE_MAX_WORKERS", 3, 1, 4)
WATCH_ANALYSIS_AUDIO_SAMPLE_RATE = _env_int("WATCH_ANALYSIS_AUDIO_SAMPLE_RATE", 16_000, 8_000, 48_000)
WATCH_ANALYSIS_AUDIO_BITRATE_KBPS = _env_int("WATCH_ANALYSIS_AUDIO_BITRATE_KBPS", 32, 24, 96)
WATCH_ANALYSIS_MAX_AUDIO_DURATION_MS = _env_int(
    "WATCH_ANALYSIS_MAX_AUDIO_DURATION_MS", 180_000, 10_000, 300_000
)
WATCH_ANALYSIS_BILIBILI_COOKIE = os.environ.get("WATCH_ANALYSIS_BILIBILI_COOKIE", "").strip()
WATCH_ANALYSIS_FFMPEG_BIN = os.environ.get("WATCH_ANALYSIS_FFMPEG_BIN", "").strip()
WATCH_SUBDL_API_KEY = os.environ.get("WATCH_SUBDL_API_KEY", "").strip()
WATCH_SUBDL_API_URL = os.environ.get(
    "WATCH_SUBDL_API_URL",
...
WATCH_SUBTITLE_LOOKUP_TIMEOUT_SECONDS = _env_int(
    "WATCH_SUBTITLE_LOOKUP_TIMEOUT_SECONDS", 45, 5, 180
)
WATCH_SUBTITLE_JOB_MAX_ATTEMPTS = _env_int("WATCH_SUBTITLE_JOB_MAX_ATTEMPTS", 1, 1, 3)
WATCH_ANALYSIS_SOURCE_USER_AGENT = os.environ.get(
...
).strip() or "@cf/qwen/qwen3-embedding-0.6b"
...
WATCH_KNOWLEDGE_SEARCH_API_URL = os.environ.get(
    ...
    os.environ.get("TAVILY_SEARCH_ENDPOINT", "https://api.tavily.com/search"),
).strip()
WATCH_KNOWLEDGE_SEARCH_API_KEY = os.environ.get("WATCH_KNOWLEDGE_SEARCH_API_KEY", "").strip() or os.environ.get("TAVILY_API_KEY", "").strip()
WATCH_KNOWLEDGE_SEARCH_TIMEOUT_SECONDS = _env_int("WATCH_KNOWLEDGE_SEARCH_TIMEOUT_SECONDS", 12, 3, 60)
WATCH_KNOWLEDGE_SEARCH_MAX_RESULTS = _env_int("WATCH_KNOWLEDGE_SEARCH_MAX_RESULTS", 3, 3, 8)
WATCH_KNOWLEDGE_TTL_SECONDS = _env_int("WATCH_KNOWLEDGE_TTL_SECONDS", 86400, 3600, 604800)
WATCH_COMPLETED_ANALYSIS_TTL_SECONDS = _env_int(
    "WATCH_COMPLETED_ANALYSIS_TTL_SECONDS", 86400, 0, 604800
)
WATCH_VISUAL_CONTEXT_ENABLED = os.environ.get("WATCH_VISUAL_CONTEXT_ENABLED", "1").strip().lower() in ("1", "true", "yes")
WATCH_VISUAL_CONTEXT_INTERVAL_SECONDS = _env_int("WATCH_VISUAL_CONTEXT_INTERVAL_SECONDS", 120, 15, 1800)
WATCH_VISUAL_FRAME_TTL_SECONDS = _env_int("WATCH_VISUAL_FRAME_TTL_SECONDS", 86400, 3600, 604800)
WATCH_VISUAL_FRAME_PAST_WINDOW_MS = _env_int("WATCH_VISUAL_FRAME_PAST_WINDOW_MS", 600_000, 60_000, 3_600_000)
WATCH_VISUAL_FRAME_FUTURE_WINDOW_MS = _env_int("WATCH_VISUAL_FRAME_FUTURE_WINDOW_MS", 300_000, 30_000, 900_000)
WATCH_VISUAL_FRAME_MAX_PER_SESSION = _env_int("WATCH_VISUAL_FRAME_MAX_PER_SESSION", 48, 4, 500)
WATCH_VISUAL_FRAME_LONG_EDGE = _env_int("WATCH_VISUAL_FRAME_LONG_EDGE", 768, 320, 1280)
WATCH_VISUAL_SHEET_QUALITY = _env_int("WATCH_VISUAL_SHEET_QUALITY", 70, 35, 95)
_WATCH_ANALYSIS_SAMPLE_DIR_STR = os.environ.get("WATCH_ANALYSIS_SAMPLE_DIR", "").strip()
WATCH_ANALYSIS_SAMPLE_DIR = (
    Path(_WATCH_ANALYSIS_SAMPLE_DIR_STR)
    if _WATCH_ANALYSIS_SAMPLE_DIR_STR
    else DATA_DIR / "watch_analysis_samples"
)
if not WATCH_ANALYSIS_SAMPLE_DIR.is_absolute():
    WATCH_ANALYSIS_SAMPLE_DIR = BASE_DIR / WATCH_ANALYSIS_SAMPLE_DIR
_WATCH_VISUAL_CACHE_DIR_STR = os.environ.get("WATCH_VISUAL_CACHE_DIR", "").strip()
WATCH_VISUAL_CACHE_DIR = (
    Path(_WATCH_VISUAL_CACHE_DIR_STR)
    if _WATCH_VISUAL_CACHE_DIR_STR
    else DATA_DIR / "watch_visual_cache"
)
if not WATCH_VISUAL_CACHE_DIR.is_absolute():
...
# 旧版兼容读取；generation v3 不再用这个值决定迁移或压缩。
SUMMARY_COMPRESSION_EVERY_N_ROUNDS = int(os.environ.get("SUMMARY_COMPRESSION_EVERY_N_ROUNDS", "8"))
SUMMARY_GENERATION_UPDATES = 8
SUMMARY_GENERATION_ROUNDS = 32
SUMMARY_STAGE_EVERY_UPDATES = 2
SUMMARY_STAGE_BATCH_SIZE = 2
SUMMARY_RECENT_TARGET_CHUNKS = 8
SUMMARY_SLIGHTLY_MAX_CHUNKS = 10
SUMMARY_OLDER_MAX_CHUNKS = 7

assert SUMMARY_GENERATION_ROUNDS == SUMMARY_EVERY_N_ROUNDS * SUMMARY_GENERATION_UPDATES
assert SUMMARY_STAGE_EVERY_UPDATES == 2
assert SUMMARY_STAGE_BATCH_SIZE == 2
...
# 旧普通有效期值仅保留给兼容读取；召回链路不再使用它过滤，普通条目只要未物理淘汰就参与召回。
DYNAMIC_MEMORY_DAYS_VALID = 10
# 卧室 tag 的短有效期只用于物理淘汰；仍在动态层中的条目不再被召回链路二次过滤。
...
_dyn_mem_review_all_merges = os.environ.get("DYNAMIC_MEMORY_REVIEW_ALL_MERGES", "0").strip().lower()
DYNAMIC_MEMORY_REVIEW_ALL_MERGES = _dyn_mem_review_all_merges in ("1", "true", "yes", "on")
...
DYNAMIC_MEMORY_RERANK_TIMEOUT_SECONDS = _env_float("DYNAMIC_MEMORY_RERANK_TIMEOUT_SECONDS", 5.0, 0.3, 30.0)
...
# 工具调用最多允许继续几轮（每继续一轮都要额外调用一次上游模型）。默认 15 以便复杂工具链收口。
TOOL_MAX_ROUNDS = int(os.environ.get("TOOL_MAX_ROUNDS", "15"))
if TOOL_MAX_ROUNDS < 1:
    TOOL_MAX_ROUNDS = 1

# 最近工具使用摘要：本地 SQLite 持久化，作为独立 prompt cache 前缀块注入。
TOOL_RESULT_CACHE_TTL_SECONDS = _env_int("TOOL_RESULT_CACHE_TTL_SECONDS", 24 * 60 * 60, 60, 7 * 24 * 60 * 60)
TOOL_RESULT_CACHE_MAX_CHARS = _env_int("TOOL_RESULT_CACHE_MAX_CHARS", 3000, 200, 20000)
TOOL_RESULT_CACHE_TRIM_TO_CHARS = _env_int("TOOL_RESULT_CACHE_TRIM_TO_CHARS", 2000, 100, 19000)
if TOOL_RESULT_CACHE_TRIM_TO_CHARS >= TOOL_RESULT_CACHE_MAX_CHARS:
    TOOL_RESULT_CACHE_TRIM_TO_CHARS = max(100, int(TOOL_RESULT_CACHE_MAX_CHARS * 2 / 3))
TOOL_RESULT_HOT_MAX_ENTRIES = 12
TOOL_RESULT_HOT_MAX_CHARS = 2000
...
_WAKEUP_STATE_DB_STR = os.environ.get("WAKEUP_STATE_DB", "").strip()
WAKEUP_STATE_DB = (
    Path(_WAKEUP_STATE_DB_STR)
    if _WAKEUP_STATE_DB_STR
    else DATA_DIR / "wakeup_state.sqlite3"
)
if not WAKEUP_STATE_DB.is_absolute():
    WAKEUP_STATE_DB = BASE_DIR / WAKEUP_STATE_DB
_DYNAMIC_MEMORY_PROVENANCE_DB_STR = os.environ.get("DYNAMIC_MEMORY_PROVENANCE_DB", "").strip()
DYNAMIC_MEMORY_PROVENANCE_DB = (
    Path(_DYNAMIC_MEMORY_PROVENANCE_DB_STR)
    if _DYNAMIC_MEMORY_PROVENANCE_DB_STR
    else DATA_DIR / "dynamic_memory_provenance.sqlite3"
)
...
RUNTIME_STATE_DB = (
    ...
    if _RUNTIME_STATE_DB_STR
    else DATA_DIR / "runtime_state.sqlite3"
)
if not RUNTIME_STATE_DB.is_absolute():
    RUNTIME_STATE_DB = BASE_DIR / RUNTIME_STATE_DB
_WENYOU_SQLITE_DB_STR = os.environ.get("WENYOU_SQLITE_DB", "").strip()
WENYOU_SQLITE_DB = (
    Path(_WENYOU_SQLITE_DB_STR)
    if _WENYOU_SQLITE_DB_STR
    else DATA_DIR / "wenyou.sqlite3"
)
...
TELEGRAM_PROACTIVE_RANDOM_MIN_MINUTES = int(float(os.environ.get("TELEGRAM_PROACTIVE_RANDOM_MIN_MINUTES", "35") or "35"))
TELEGRAM_PROACTIVE_RANDOM_MAX_MINUTES = int(float(os.environ.get("TELEGRAM_PROACTIVE_RANDOM_MAX_MINUTES", "55") or "55"))
if TELEGRAM_PROACTIVE_RANDOM_MIN_MINUTES < 1:
    TELEGRAM_PROACTIVE_RANDOM_MIN_MINUTES = 1
if TELEGRAM_PROACTIVE_RANDOM_MAX_MINUTES < TELEGRAM_PROACTIVE_RANDOM_MIN_MINUTES:
    TELEGRAM_PROACTIVE_RANDOM_MAX_MINUTES = TELEGRAM_PROACTIVE_RANDOM_MIN_MINUTES
...
MINIMAX_VOICE_EMOTION = os.environ.get("MINIMAX_VOICE_EMOTION", "").strip()
...
MIJIA_SPEAKER_DID = os.environ.get("MIJIA_SPEAKER_DID", "").strip()
