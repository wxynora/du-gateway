...
def tail_file_lines(path: str, lines: int = 200, encoding: str = "utf-8") -> list[str]:
    """
    读取文件末尾 N 行。
...
def tail_logs(path: str, lines: int = 200) -> list[str]:
    """
    ...
    if path:
        ...
        if not p.is_absolute():
            ...
            p = base_dir / path
        if p.exists():
            return tail_file_lines(str(p), lines=lines)
    return tail_buffer_lines(lines=lines)


def stream_logs_sse(path: str, start_lines: int = 80, poll_interval_s: float = 0.5):
    """
    ...
    if path:
        ...
        if not p.is_absolute():
            ...
            p = base_dir / path
        if p.exists():
            yield from stream_file_tail_sse(str(p), start_lines=start_lines, poll_interval_s=poll_interval_s)
        return
