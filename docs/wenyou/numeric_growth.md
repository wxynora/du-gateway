...
默认固定使用六项基础属性。攻击、防御、精神抗性、先手、负重等不作为自由加点属性，而是由基础属性、当前状态、阶位、背包道具、能力和进化派生。
...
| `spi_max` | 当前精神力上限，由 `spi`、阶位、进化、道具和状态修正得到 |
| `level` | 等级，长期成长 |
| `rank` | 阶位，D/C/B/A/S |
| `exp` | 经验 |
| `points` | 主神积分 |
...
| 派生值 | 公式 | 用途 |
| --- | --- | --- |
| `physical_attack` | `floor(str / 2) + ability_attack + evolution_attack + item_attack_bonus` | 近战、破坏、物理压制 |
| `ranged_attack` | `floor((agi + int) / 4) + ability_attack + item_attack_bonus` | 远程、投掷、精准射击 |
...
- 体力变化后，后端立刻重算 HP 上限。
- 智力变化后，后端立刻重算 SAN 上限。
- 精神力基础值、阶位、进化、道具或状态变化后，后端立刻重算 `spi_max`。
...
| 能力/进化 | 通关奖励、隐藏奖励、抽卡、特殊道具解封 | 主动/被动能力、身体方向、特性、抗性或副作用 | 槽位、阶位封印、次数、冷却、代价 |
...
- 按公式重算 HP/SAN 上限。
- 当前 HP/SAN 不自动回满。
- 升级不再发能力点、成长令牌或进化碎片；能力和进化只来自随机奖励、隐藏奖励、抽卡或内容包明确结算。
...
- 默认先不提供常驻属性重置；以后如果加重置，只返还自由属性点，不返还属性药剂或进化提供的永久点。
...
- 晋升本身不额外发属性点；阶位已经提供上限、减伤、解封和商店权限。
...
| B 成长期 | Lv8-10 | 18-24 | B | 能力、进化和背包道具开始决定打法 |
...
下面是“普通成长 + 少量背包道具/能力/进化加成”的期望区间，用于生成怪物面板时校准，不是玩家属性硬限制。
...
- 玩家不能从能力目录里自主挑选学习。
- 能力只通过通关奖励、隐藏奖励、抽卡、特殊道具解封或内容包明确结算获得。
- 抽卡或奖励得到的是具体能力模板；使用模板后绑定到角色，不再额外消耗能力点或碎片。
- 能力没有默认 Lv1-Lv5 升级链。强度由稀有度、阶位封印、使用次数、冷却和代价控制。
- 重复能力默认转化为命运裂隙残片或替代奖励，不直接返还积分。
...
| ID | 稀有度 | 名称 | 固定效果 | 限制/代价 |
| --- | --- | --- | --- | --- |
| `quick_bandage` | D | 快速包扎 | 每副本 1 次，非战斗恢复 20 HP | 不能在濒死倒计时中使用 |
| `steady_breath` | D | 稳定呼吸 | 每副本 1 次，恢复 20 SAN，并同步恢复少量当前精神力 | 不能移除污染 |
| `anomaly_intuition` | D | 异常直觉 | 每副本 1 次，获得一个轻微异常提示 | 不保证指出解法 |
| `danger_premonition` | C | 危险预感 | 危险事件前获得一次提示 | 每副本 1 次 |
| `short_tracking` | C | 短距追踪 | 标记目标 3 轮，追踪判定 +3 | 目标换场景后可能失效 |
| `mental_anchor` | C | 精神锚点 | 抵消或移除一次 `动摇` | 不能处理重度污染 |
| `rule_probe` | B | 规则试探 | 每副本 1 次，验证一条低级规则 | 结果可能是真、假或不完整 |
| `shadow_hide` | B | 影中藏身 | 潜伏行动风险 -1 级 | 不能对抗强制追猎 |
| `damage_shift` | B | 伤害转移 | 每副本 1 次，将 50% HP 伤害转为 SAN 伤害 | SAN 不足时不可用 |
| `causal_rollback` | A | 因果回滚 | 每副本 1 次，撤销本轮一次非死亡后果 | SAN -35 |
| `identity_disguise` | A | 身份伪装 | 潜伏调查暴露度 -2 | 暴露后本副本失效 |
| `pollution_immunity` | A | 污染豁免 | 抵消一次 A 级以下精神污染 | 每副本 1 次 |
| `death_denial` | S | 拒绝一次死亡 | 每 3 个副本 1 次，死亡时保留 1 HP，SAN 归 0 | 触发后写入契约/债务风险 |
| `minor_rule_rewrite` | S | 低级规则改写 | 改写一条低级公开规则 | 威胁时钟 +2 |
| `settlement_audit` | S | 强制结算复核 | 结算时重算一次奖励或死亡判定 | 必须接受新结果 |
