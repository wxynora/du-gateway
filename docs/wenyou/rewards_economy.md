...
低完成度撤离时可保留部分保底；纯失败不发积分，死亡、复活、债务和装备损耗会形成实际成本。
...
| 装备损耗 | 当前装备耐久额外 -10 到 -30，或随机损坏 1 件道具 |
...
penalty_points = revive_costs + debt_costs + abandon_penalty + repair_costs
...
最终积分不得低于 0；倒扣优先扣现有积分，不够的部分写入 `debts`，不直接让 `points` 变负。`死亡失败` 默认 `gross_points = 0`，因此只会扣复活、债务、维修等成本。
...
| 稀有度 | 消耗道具 | 锻造材料 | 武器/装备 | 能力碎片 | 进化碎片 | 特殊物/称号 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| D | 45% | 25% | 15% | 10% | 5% | 0% |
| C | 30% | 25% | 20% | 15% | 8% | 2% |
| B | 18% | 24% | 24% | 18% | 12% | 4% |
...
| 锻造材料 | 低级锻材 x1 | 低级锻材 x2 或异常残片 x1 | 中级锻材 x2 或异常核心 x1 | 高级锻材 x2 或副本核心 x1 | 传说锻材 x1 或副本核心 x2 |
| 武器/装备 | D 装备 x1 | C 装备 x1 | B 装备 x1 | A 装备 x1 | S 装备 x1 或 S 武器胚 x1 |
| 能力碎片 | 10 | 25 | 60 | 160 | 500 |
| 进化碎片 | 8 | 20 | 50 | 140 | 450 |
...
- 同一结算中如果连续 3 次奖励都是材料，第 4 次强制改为道具、装备、能力碎片或进化碎片。
...
| --- | --- |
| D | 低级锻材、随机 D 武器箱 |
| C | 低级锻材、随机 C 武器箱、异常残片 |
| B | 中级锻材、随机 B 武器箱、异常核心 |
| A | 高级锻材、A 武器胚、低概率副本核心 |
| S | 传说锻材、S 武器胚、副本核心、唯一武器 |
...
| 回收异常 | 取回系统遗失道具、异常碎片或债务凭证 | 污染上升、装备耐久损耗 |
