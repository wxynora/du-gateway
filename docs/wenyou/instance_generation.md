...
默认内容包的第一局固定为手写副本 `T-000 白箱回廊`，不走随机副本生成。

定位：

- 低危、短流程、无任务者 NPC、无怪物。
- 直接以小说式开局进入副本，而不是先进入主神空间选教程。
- 三段结构：醒来、灯色规则、出口。
- 每段都允许多种通过方式，目的不是卡关，而是观察玩家自然倾向。
- 通关后再开放主神空间、候选副本、商店、抽卡、背包、角色面板等完整功能。

白箱回廊的行为测试方向：

| 倾向 | 典型行动 |
| --- | --- |
| 观察调查 | 停下、打量、检查墙面/光轨/门 |
| 脱身潜行 | 试图绕开、冲过、快速通过、寻找退路 |
| 保护支援 | 先确认同伴状态、拉住同伴、承担风险 |
| 正面破局 | 砸面板、撬门、硬闯、破坏障碍 |
| 社交伪装 | 询问系统、和同伴沟通、试图谈判 |
| 规则试探 | 主动验证红白灯、推理条件、测试边界 |
| 抗压存活 | 在回弹、眩晕或噪声后保持冷静继续推进 |

这些倾向只用于新手副本通关后的 `core_ability` 与 `core_ability_profile` 生成，不改变抽卡概率，也不直接给额外奖励。
