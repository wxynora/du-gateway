...
当前后端提供一层轻量工具桥，方便别的 AI 后端或官方 App LLM 读取上下文并执行工具。它不绑定具体模型，也不要求对方读取全量后端表。

接口：

```text
GET  /miniapp-api/wenyou/mcp/tools?actor_id=player2
POST /miniapp-api/wenyou/mcp/tool-call
GET  /miniapp-api/wenyou/mcp/sse?actor_id=player2
```

`GET /wenyou/mcp/tools` 返回：
...
`POST /wenyou/mcp/tool-call` 请求格式：
...
`GET /wenyou/mcp/sse` 是一条初始化流，按顺序发送：
