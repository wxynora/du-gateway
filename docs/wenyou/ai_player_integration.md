...
它不是 NPC 好感系统，也不是 GM 权限扩展。AI 玩家是一个真实玩家队伍里的独立玩家角色：有自己的角色卡、钱包、背包、抽卡保底和消费流水。GM 仍负责叙事，Rules Engine 仍负责精准数值和状态写入。
...
- 让 AI 玩家拥有自己的积分、债务、背包、抽卡保底和消费记录。
...
AI 玩家不能直接修改其他玩家角色的钱包、背包、属性或行动结果。跨角色转交必须通过规则工具完成。
...
3. AI 玩家阅读只读上下文
4. AI 玩家决定是否行动
5. 若本轮参与行动，由 AI 玩家链路自己产出行动文本，并作为 `ai_player_action` 交给回合推进接口
6. 若要改状态，调用玩家工具
...
后端不调用 GM/DS 代写 AI 玩家行动；GM 只在真实玩家行动和已给定的 AI 玩家行动都进入 pending round 后续写剧情。
...
当前后端提供一层轻量 HTTP 工具桥，方便别的 AI 后端或官方 App LLM 读取上下文并执行工具。它不绑定具体模型，也不要求对方读取全量后端表。

这不是标准 MCP 服务，也不要求接入方实现 MCP client。它只是文游自己的 `wenyou-ai-player-tools-v1` 协议：读取上下文、拿工具 schema、提交工具调用。

接口：

```text
GET  /miniapp-api/wenyou/ai-player/tools?actor_id=player2
POST /miniapp-api/wenyou/ai-player/tool-call
GET  /miniapp-api/wenyou/ai-player/sse?actor_id=player2
```

旧的 `/miniapp-api/wenyou/mcp/*` 只作为兼容别名保留，不作为开源推荐入口。

`GET /wenyou/ai-player/tools` 返回：

- `protocol: "wenyou-ai-player-tools-v1"`
- `tools`: 固定工具 schema，等价于 `get_player_tool_schemas()`
- `context`: 当前 AI 玩家只读上下文，等价于 `get_player_tool_context(actor_id)`
- `tool_call_url`: 工具执行入口
- `sse_url`: SSE 初始化流入口

`POST /wenyou/ai-player/tool-call` 请求格式：
...
返回里 `result.ok`、`result.message`、`result.state_patch` 和 `result.ledger_entry` 是规则层事实源。外部 LLM 不能把失败工具当成功写进剧情。

`GET /wenyou/ai-player/sse` 是一条初始化流，按顺序发送：
...
这条 SSE 只用于把工具清单和当前上下文流给外部 AI 后端；不会自动推进回合，也不是 GM 剧情流。需要修改状态时仍调用 `tool-call`；需要让 AI 玩家参与本轮行动时，外部后端产出行动文本后交给 `/wenyou/action`、`/wenyou/item/use` 或 `/wenyou/encounter/action` 的 `ai_player_action` / `player2_action` 字段。

## 四类状态修改工具

默认只保留四类状态修改入口。读取状态、查看背包、查看商店、查看角色面板都不算工具调用。

为了让 AI 玩家更容易稳定调用，可以额外暴露 `use_item` 作为快捷工具；它只是 `inventory_action(action="use")` 的别名，不新增规则能力。
...
- 购买失败不扣积分。
- AI 玩家不能用该工具给其他玩家直接购买，买完后可另用 `transfer` 转交。
...
{
  "ok": true,
  "message": "玩家二购买【初级精神药剂】，花费 30 积分。",
  "state_patch": {
    "source": "player_tools.buy_item",
    "actor_id": "player2",
    "changes": {
      "wallets": {
        "player2": {
...
{
  "ok": true,
  "message": "玩家二使用【初级精神药剂】，恢复 30 SAN。",
  "state_patch": {
    "source": "player_tools.inventory_action",
    "actor_id": "player2",
    "changes": {
      "players": {
        "player2": {
...
```text
AI 玩家意图 -> 调用 buy_item -> Rules Engine 返回结果 -> GM/前端展示结果
```
...
你可以阅读自己的角色面板、钱包、背包、商店货架、抽卡保底和最近消费流水。
...
你可以自由使用自己的积分买东西、抽卡、出售和转交物品，不需要真实玩家确认。
你不能直接花其他玩家的积分，不能直接改其他玩家的背包，不能替其他玩家决定行动。
你想修改状态时，必须调用可用的玩家工具，并接受工具返回结果。
你不能在叙事里宣称工具尚未确认的购买、抽卡、治疗或转交已经成功。
如果当前阶段不允许商店、抽卡或转交，你应该等待、行动或说明原因，而不是假装完成。
```
...
| 阶段 | AI 玩家可做 |
| --- | --- |
| `hub` | 买商店、抽卡、出售、转交、查看面板 |
| `candidate_selection` | 查看面板、整理非高压物品；默认不买商店，不推进副本剧情 |
| `instance_running` | 使用道具、转交允许物品、查看面板；不能买系统商店或抽卡 |
| `settlement` | 买商店、抽卡、出售、转交、结算收入入账 |
...
buy_item(state, actor_id, offer_ref | item_id, quantity) -> state_patch
roll_gacha(state, actor_id, pool_id, count) -> state_patch
use_item(state, actor_id, item_ref, target_id, context) -> state_patch
inventory_action(state, actor_id, action, item_ref, target_id, context) -> state_patch
...
- `use_item` 是推荐暴露给 AI 玩家模型的快捷入口，内部可以直接调用 `inventory_action(..., action="use")`。
...
3. 旧 `stats.inventory` 迁到 `inventories.player1`，除非物品已有 `holder_id`。
