...
### Phase 3 当前落地（2026-06-09）

已落地：

- 新增 `miniapp/src/ui/chat/chatSendStage.ts`。
- 前端把 `chat_send_start`、`chat_job_create_*`、`chat_job_status`、`chat_reply_ready`、取消/错误等事件映射成短阶段文案。
- `MainChatScreen.tsx` 维护 `activeSendStageLabel`，发送中在顶部显示一个小状态胶囊，例如“准备发送”“提交给后端”“任务已创建，等渡回复”“正在请求上游”“正在写入回复”“正在取消”。
- 私聊、普通群聊 Du reply、自由讨论接力里的 Du reply 都复用同一套阶段显示。

未做：

- 未做 SSE/流式。
- 未做连续发送队列。
- 未改持久化 `cancelled` 状态或 Android SQLite schema。
- 未改后端 job 协议，只消费已有日志事件和 job stage。

## Phase 4：retry / recovery 拆离

retry 和 recovery 不是同一件事：

- 后台恢复：优先接回 operation 里已有的 `jobId`，如果 job 不存在或过期，再用 `retryPayload` 重新创建。
- 手动重试：复用原 `operationId` / `clientRequestId` / `retryPayload`，但不继续死等旧失败 `jobId`，直接重新 post。

### Phase 4 当前落地（2026-06-09）

已落地：

- 新增 `miniapp/src/ui/chat/chatRecoveryFlow.ts`。
- `recoverSumiTalkOperationFlow` 统一处理已有 job 恢复、job 过期后重建、direct done、job poll、assistant terminal / failed message、voice sidecar 触发。
- `MainChatScreen.tsx` 的 `recoverSumiTalkOperation` 改为只负责 operation 去重、消息持久化回调和 sidecar 回调，不再内联 post/poll/terminal 构造。
- 手动点击“重试”会传 `forceCreateJob=true`，清掉气泡里的旧 `jobId`，复用原请求重新创建 job；自动恢复不传该参数，仍优先接回已有 job。
- recovery 继续不走前台 `attemptId` guard，仍按 `operationId/clientRequestId` 授权更新 pending。

未做：

- 未改 Android SQLite schema。
- 未新增独立 outbox 或发送队列。
- 未把笨笨 task recovery / realtime fallback 拆出主页面。
- 未做 SSE/流式。
