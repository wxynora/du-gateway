...
看网关的**同一条日志输出**（上面说的终端或 `tail -f gateway.log`），发一条对话后搜下面关键词：

| 日志里出现 | 说明 |
|------------|------|
| **R2 已写入 对话轮次** | 说明本轮已成功写入 R2，若桶里仍看不到可检查桶名/账号是否一致。 |
| **R2 client 未配置，跳过 append_conversation_round** | .env 里没配 R2（`R2_ACCOUNT_ID`、`R2_ACCESS_KEY_ID`、`R2_SECRET_ACCESS_KEY` 至少缺一个），不会写 R2。 |
| **append_conversation_round 失败** 或 **R2 write_json 失败** | 已连 R2 但写入报错，后面会跟具体 error，按错误修（权限、桶名、网络等）。 |
| **R2 未存档：上游无 choices 或响应为空** | 上游 API 返回里没有 `choices`，网关不存档。 |
| **R2 未存档：上游回复被判为失败** | 回复太短或含错误关键词，被判定为失败对话，不存档。 |

若一条 R2 相关日志都没有，说明请求可能没走到「完整管道」或没走到存档那一步，检查前面是否有转发失败、status>=400 等。
