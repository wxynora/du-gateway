...
- 每轮聊天未来注入上限：`message_playhead + 2 分钟`。动态 system 明确写入“不要和小玥照搬复述你看到的剧情内容”，剧情文本只用于理解共同观看位置和自然回应。
...
3. 开播前已经准备并持久化的独立字幕轨；每个截图样本聚合到下一采样点之前的字幕。Bilibili 原生字幕为空且配置了 `WATCH_SUBDL_API_KEY` 时，只用准备阶段确认的 `original_title + year` 搜索一次任意语言字幕，并按人工正片起点换算到媒体时间。字幕网络请求不复用视频取材的长超时：默认每次最多 15 秒、一次准备总计最多 45 秒，候选下载 URL 保序去重但不限制唯一候选数量；任务默认只自动尝试一次，终态失败后由准备页显式重试。worker 只记录搜索、下载阶段耗时和状态，不记录字幕正文、Cookie 或 key。字幕只是辅助证据，不为它增加独立模型对齐请求；正片删减或版本差异导致字幕与实际音频、画面冲突时，Gemini 必须忽略冲突字幕并以实际音画为准。rolling 不再临时搜索 SubDL；字幕不可用时按使用者在准备页确认的结果降级为音频与截图主链。
...
- 字幕准备使用独立的单次请求和总耗时预算、候选 URL 去重、一次自动尝试与阶段耗时日志；弹幕提示按预计回复抵达位置选择可靠未来片段和示例时间，并保留原有合法动作放行规则。
