...
网关返回的 `fear_protection.status` 负责 `off | protected | coverage_low`，并同时给出 `coverage_remaining_ms`、`required_buffer_ms`、`required_until_ms` 和原因。`covering | user_bypassed` 是 App 根据当前风险事件、遮挡动作和使用者选择维护的播放器本地状态，网关不能替前端假报。
...
滚动解析从 Gemini 请求源头按使用者选择的 `knowledge_mode` 使用两套明确的 system 约束。`known` 模式要求所有任务的 `story_so_far.background` 固定为空字符串，不生成陪伴者剧情背景，也不把旧版本留下的 background 送回模型续写；`needs_summary` 模式才要求 rolling 根据固定知识卡、已确认前序状态和本批实际素材整理 background。两种模式都继续维护 `story_so_far.summary` 和 `story_state`，因此“不产出陪伴者背景”不会破坏分析器自身的剧情连续性。
...
背景卡不能原样复述给陪伴者。滚动解析 Gemini 只使用它辅助认人、理解世界观和开场前情；实际剧情仍以音频、截图、字幕和已确认前序状态为准。只有 `needs_summary` 模式会在 `story_so_far.background` 中输出截至 `through_ms` 的防剧透剧情背景，`known` 模式从分析源头保持该字段为空。
...
`work_knowledge_card`、`story_so_far.background` 和 `analysis_story_state` 的职责不能混用：背景卡是开播前生成的简短固定参考，background 是仅在 `needs_summary` 下随解析进度输出的防剧透检查点，story state 是两种模式都随播放推进的分析器内部动态状态。
...
剧情背景只由 `needs_summary` 模式的滚动解析 Gemini 输出，给陪伴者时不使用内部字段名或 JSON。网关读取 `through_ms <= message_playhead_ms` 的最新 `story_so_far.background`，渲染为可选的“剧情背景”段并放在“当前剧情”之前；`known` 模式或没有可靠检查点时整段省略。
...
- 按 `knowledge_mode` 从 Gemini 源头分流的 `story_so_far.background`、当会话已播片段召回、每会话回复抵达窗口、`[du:danmaku ...]` 短标记和旧长块解析兼容。
