...
| --- | --- | --- | --- | --- | --- |
| `watch-explicit-end-ticket-20260722` | 已完成 | 施工；证据读取与精确写入仅 `routes/miniapp/watch.py`、`storage/watch_runtime_store.py`、`storage/watch_viewing_store.py`、`storage/stay_with_du_store.py`、`scripts/test_watch_viewing_ticket.py`、`docs/SumiTalk一起看方案.md`、`docs/CURRENT_WORK.md`、`docs/DEBUG_INDEX.md`；不改 Android、分析调度、其他 Stay with Du 行为、R2 数据或其他脏文件 | 正片完成与明确结束已分离：普通 session 收尾不出票；`finalize_viewing=true` 无论是否到正片终点都按服务端可信累计生成稳定票根；到正片终点后继续播放仍累计；票根支持修改最终片名，作品确实看完后才可由使用者选择归档为一起看过 | 无；运行改动待提交、推送并部署 | 四组一起看定向回归通过；运行文件 `py_compile`、`import app` 与限定范围 `git diff --check` 通过；测试隔离外部写入，未写 R2 |
