...
| --- | --- | --- | --- | --- | --- |
| `watch-remove-knowledge-cost-caps-v1` | 已完成待部署 | 修复一起看滚动分析追着播放头产生超短批次的费用失控；首段门锁仍准备 5 分钟，后台随后按完整批次顺序解析至最多领先 30 分钟或正常正片结尾 `content_end_ms`；删除知识卡显式输出上限和每日分析费用门槛；保留单任务最多 3 次尝试；同步 Lean In，不改 Android | 旧的差值补批和费用延期路径已删除；只有完整批次或到 `content_end_ms` 的最后收尾批次能调用模型；Lean In 已先推送 `71c3f5b` | 提交并部署网关运行代码后重启网关与分析 worker | 三组定向测试、入口导入、`py_compile` 和 `git diff --check` 通过；worker 仍停止；不写 R2 |
