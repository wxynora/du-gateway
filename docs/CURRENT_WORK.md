...
| --- | --- | --- | --- | --- | --- |
| `watch-viewing-ticket-backend` | 实现完成待部署 | 仅网关一起看后端：可信播放累计、跨分 P viewing、真实完成门禁、幂等票根与跨设备票夹恢复；不改 Android、不改 Stay with Du、不改分析与费用策略 | 已新增长期 viewing 聚合、服务端校准后的实际播放时长、跨 P 完成状态与稳定结构化票根；DELETE 只收尾并保留 analysis_cost，不会冒充看完；误加的 Stay with Du 代码和 R2 链路已完全撤掉 | 随本轮一起看后端提交推送并部署两项服务 | 四条一起看定向回归及目标模块 py_compile 通过；新回归覆盖暂停、倍速、seek、跨 P、完成门禁、重复 DELETE、票根恢复和老库迁移；未调用外部 provider 或 R2 |
| `watch-cost-high-risk-hardening` | 实现完成待部署 | 仅网关一起看后端：建会幂等、seek 跨 epoch 复用已付费剧情区间、完整费用账；不改 Android、不改重试次数、不改 30 分钟预取 | 建会幂等、已付费媒体区间跨 epoch 复用、完整 provider 费用账和人工参考范围语义均已落盘；真实 provider 响应先按事件幂等落账 | 随本轮一起看后端提交推送并部署两项服务 | Lean In 已以 `6db969b` 推送；网关四条定向回归与目标模块编译通过，测试均使用本地假上游 |
