...
| --- | --- | --- | --- | --- | --- |
| `gateway-ship-completed-20260721` | 进行中 | 仅收束并发布当前已完成的 6 个后端运行文件与 `docs/CURRENT_WORK.md`、`docs/DEBUG_INDEX.md`；测试文件不提交、不部署；不改 R2 数据、原生 App 或隔壁改动 | 限定 diff 与精确 staged 版本均已复核通过，准备提交到 `main` 并部署重启 | 提交、push，远端 `git pull --ff-only` 后重启相关服务并检查健康与日志 | 干净 detached worktree 叠加 staged diff 后，SumiTalk rich stream、动态 merge parser、动态记忆 prune、睡眠日期、6 文件 `py_compile`、`import app` 与 `git diff --check` 均通过；未调用 DS、未写 R2 |
| `sumitalk-reasoning-rich-order-20260721` | 已完成 | SumiTalk 流式 rich event 中 reasoning part 预留、正文即时发送、reasoning 结束边界与对应回归测试；读写范围扩到 `routes/chat.py` 的 SumiTalk rich event emit 段；不改 App、MiniApp UI、通话链路、R2 或提示词 | 启用 reasoning 的请求会在流开始时预留稳定 reasoning part；首次正文到达前立即结束该 reasoning 阶段并继续发送正文；后到 reasoning 只更新同一 part，不在正文后新建 reasoning part；测试已区分上游原始交错顺序和 rich event 逻辑展示顺序 | 无；等待按现有发布流程提交/部署 | `.venv/bin/python scripts/test_sumitalk_native_stream_backend.py`、`py_compile`、限定范围 `git diff --check` 通过 |
