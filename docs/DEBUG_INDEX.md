...
| 工具使用摘要缓存 | `services/tool_result_cache.py`、`services/model_token_ratio.py`、`storage/runtime_sqlite.py` | 工具循环结束后一次性写本地 SQLite；结果按工具清洗，不保存原始大 JSON；每个模型第一次真实使用时按实际上游请求体字符数与返回的 input/cache token 学习并锁定换算比例，后续直接复用；24 小时 TTL，超过约 5k tokens 时按完整旧记录收缩到约 3.5k |
...
- 工具摘要表：`tool_result_cache`；模型字符/token 首次样本表：`model_token_ratios`（均在同一运行 SQLite；非 R2）
