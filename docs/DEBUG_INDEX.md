...
- 已实测：lv2 夜间无模仿者卡槽 `小喷菇,小喷菇,向日葵,大喷菇,窝瓜,樱桃炸弹` 在小工玩家手里可于第 18 回合、tick 1142 通关，说明 `小喷菇(0)` 直种重复卡槽会绕过“无模仿者经济滚不起来”的设计。已将直种小喷菇费用从 0 调到 25；随机开奖出的小喷菇不受影响。
- 已完成：开局选卡界面新增默认卡组 `模仿者 模仿者 模仿者 模仿者 向日葵 窝瓜`，并支持 `cards 默认` / `cards default` 使用该卡组；默认仍不是强制，玩家可自行编辑。同步补 `plant_triggered` 玩家事件文案，例如 `4路6列樱桃炸弹触发（消灭2只）`，避免即时植物只出现在“新单位”里。
- 已实测：调整后同一无模仿者旧开局动作会在第 4 个种植动作因阳光不足中断；默认四模仿者 lv2 新小工局 seed `lv2-night-default-imitator-20260703-a` 前 5 回合到 tick 272，系统波次 3/11，场上 4 僵尸、10 植物、2 个未开奖模仿者。小工反馈随机版味道明显增强，压力合理，小喷菇 25 后不太会再主动无模仿者开局。
- 已实测：同一默认四模仿者 lv2 小工局继续推进到第 32 回合、tick 1472，系统波次已在 tick 770 结束，但玩家继续种模仿者导致残局反复延长，并在第 30 回合自触发僵王博士事件；期间用掉 1/2/3/4 路推车，场上仍剩 2 只僵尸和 1 个 Boss 事件。结论：lv2 明显比 lv1 难，默认卡组能把玩法推向随机版；系统波次结束后继续种模仿者属于玩家自选风险和乐趣，不应削掉，复盘经验应引导玩家在残局阶段只对危险路补模仿者。
- 已修复：玩家视图兼容 `plant_triggered` 事件里的实体植物 id，优先展示 `plant_type` 与坐标，避免窝瓜/土豆雷等触发时出现 `None路None列p61触发` 这类内部信息。
