...
| 后台模型岗位表 | `services/worker_models.py` | `translate / structured / ocr / asr / rerank` 统一返回 provider、协议、URL、密钥和 model；同协议换模优先覆盖对应 `WORKER_*_MODEL`，换 provider 时同处覆盖 provider/URL/key。translate 当前为硅基混元 MT，structured 当前为硅基 Qwen3-8B 并接网页压缩、工具摘要和动态层 new / merge / skip 判断，ocr 当前通过 Google AI Studio OpenAI compatibility 使用 `gemini-3.5-flash-lite`，仅从 `WORKER_OCR_*` 读取配置；asr 保留 Deepgram/OpenRouter 双适配与既有主备顺序，rerank 沿用硅基 Qwen3-Reranker-8B。一起看知识卡仍保持 DS V4 Flash，待独立效果对比后再决定是否接 structured 岗位；新协议仍须新增对应适配器，不把协议差异伪装成只换 model ID |
