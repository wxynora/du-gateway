...
| 后台模型岗位表 | `services/worker_models.py` | `translate / structured / background_reasoning / ocr / asr / rerank` 统一返回 provider、协议、URL、密钥和 model；同协议换模优先覆盖对应 `WORKER_*_MODEL`，换 provider 时同处覆盖 provider/URL/key。translate 当前为硅基混元 MT；structured 当前为硅基 Qwen3-8B，只接网页压缩和工具摘要；background_reasoning 当前为硅基流动 `deepseek-ai/DeepSeek-V4-Flash`，接动态层 new / merge / skip、身体状态评估、中期记忆、记忆维护去重、人工记忆重写和近期总结，以上链路不再回落 DeepSeek 官方 V4 Flash；ocr 当前通过 Google AI Studio OpenAI compatibility 使用 `gemini-3.5-flash-lite`，仅从 `WORKER_OCR_*` 读取配置；asr 保留 Deepgram/OpenRouter 双适配与既有主备顺序，rerank 沿用硅基 Qwen3-Reranker-8B。一起看知识卡仍保持现有硅基 DS V4 Flash 独立链；查询改写的 `deepseek-v4-pro`、文游与主模型链不属于 background_reasoning；新协议仍须新增对应适配器，不把协议差异伪装成只换 model ID |
