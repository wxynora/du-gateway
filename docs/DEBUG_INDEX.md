...
- 已完成：`services/upstream_policy.py` 会给 Pioneer Claude 请求的最后一个 `tools` 条目补 `cache_control: {type:"ephemeral", ttl:"1h"}`，对应本地 Claude OAuth proxy 的 tools 断点；只在 `is_pioneer_url(...)` 且 active/model 为 `claude-*` 时生效，不影响普通上游。
- 已完成：`routes/chat.py` 对 Pioneer Claude 非流式请求内部改走 Pioneer Anthropic-compatible `POST /v1/messages`；`.env` / API 管理里仍保留 OpenAI-compatible `/v1/chat/completions` 作为配置入口，代码转发前转换 OpenAI messages/tools/tool_calls/tool results 到 Anthropic Messages，请求回来后再转回网关现有 OpenAI `choices[0].message` 结构，避免影响工具循环和存档链路。
