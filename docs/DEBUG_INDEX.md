...
- 已验证：目标提交的格式归一化 6 项和网易云登录/推荐/FM/源/路由边界 14 项测试在本机与 VPS 均通过；PyNCM 固定提交可导入，新增推荐/FM 的底层路径与 PyNCM 装饰器契约一致，19 条网易云路由已注册，相关 Python 编译、脚本 `--help` 与 `git diff --check` 通过。测试全部使用假登录态、假曲源或本地合成音频，没有扫码真实账号、下载真实歌曲、调用 Gemini 或写 R2。
- 已部署：提交 `f87c6793` 已推送 `origin/main` 并由主网关 fast-forward；VPS `.venv` 已安装 PyNCM 1.8.1，系统已安装 ffmpeg/ffprobe 6.1.1。`du-gateway.service` 重启后 active，本机和公网 `/health` 均返回 `ok`；未鉴权访问推荐/FM 返回 401，带临时面板 Token 时 `/status` 返回未登录，推荐/FM 返回“网易云尚未登录”，证明路由与鉴权已生效且未触发真实网易云调用。重启后日志无 traceback/error。
- 配套原生端：网易云扫码、搜索/歌单、播放/歌词、显式“分析并留存”和「渡的歌单」置顶仍在独立原生 worktree，尚未合并、推送、安装或真机联调。后端不会自动创建或修改网易云歌单；首次真实扫码必须由辛玥在原生 App 内明确操作。
...
- 已验证：合法首日安排一次同步后落盘，普通 `status` 可读回第一段；非法安排明确失败且存档仍等待安排；原有部分成功重试语义未受影响。初期完整回归曾导入 `r2_store`，其写入函数已被测试替换为空操作，未发生 R2 读写；用户指出边界后，发布验证只运行 Python 静态编译、MiniApp TypeScript、生产构建和 `git diff --check`，不再加载 R2 模块。
- 已部署：提交 `72990288` 已推送 `main` 并由服务器 fast-forward；服务器静态编译通过，`du-gateway.service` 与 `du-sumitalk-chat-worker.service` 重启后均为 active。公网 `/health` 返回 `ok`，`/miniapp/` 已引用 `index-COl6UgNq.js`，新囚禁模拟器 chunk 返回 200，重启后日志未发现异常。
- 边界：发布与部署检查未调用游戏接口或线上模型，也未加载、读取或写入 R2；服务器未跟踪的备份、日志和运行库未改动。

当前状态（2026-07-13 囚禁模拟器礼物夜间统一发放）：
- 已完成：两条路线的赠送都只写入待发放队列，不单独触发或续接同步，也不占白天行动；任意时段送出的礼物在可进入夜间选择时统一发放，附言随礼物出现，可交互物品同时加入当晚行动菜单。
- 已完成：白天待发礼物、彩蛋内容和语音铃台词只对囚禁方可见；仓库区分“待发放 / 已赠送”，可在发放前取消。给渡的夜间提示写“小玥送了你”，小玥的夜间面板写“渡送了你”，书名统一显示为《书名》。
- 已验证：私有版完整 `scripts/test_captivity_simulator_game.py`、MiniApp TypeScript、相关 Python 编译和 `git diff --check` 通过；开源提交 `b94c036` 在干净 worktree 通过 30 项 unittest、Web 生产构建和开源占位审计，已推送 `main`。
- 边界：本轮未访问 R2、未请求线上模型、未运行线上实测、未部署或重启服务。私有改动仍在临时 worktree，尚未提交或推送；已有 `miniapp_static` 构建哈希变动不属于本次逻辑修改，后续发布前需单独确认。

当前状态（2026-07-13 囚禁模拟器高风险链路闭环复查）：
- 已修复：后端自动续接改为最多 3 步的明确白名单，补齐“监控不看/看后静默 -> 次日安排”“语音铃不过去 -> 次日安排”“一次夜间行动连续发现两项物品彩蛋”“夜间结算 -> 第二天逃跑诱导”“逃跑选择 -> 抓回经过”等必要链路；普通白天行动不会因此多轮调用。
- 已修复：同步时只有权威存档中确实轮到渡的 pending 才能被渡的指令推进；轮到小玥操作时，渡回复里的行动、监控或回应指令不能越权改状态。赠送/收回物品仍是行动外行为，但前端只在原本就轮到渡时继续已有同步，不会因送礼额外打断本地选择。
- 已验证两条路线：渡囚禁小玥的监控跳过、监控介入、语音铃跳过、次日安排与行动回应；小玥囚禁渡的夜间行动、连续物品彩蛋、本地监控介入与过程回写；两边都覆盖玩家 pending 防越权及送礼不改 pending。全部使用临时本地存档和假 AI。
- 验证结果：私有版完整 `scripts/test_captivity_simulator_game.py`、Python 编译、MiniApp TypeScript 检查、`git diff --check` 通过；开源版 29 项 unittest、开源审计、Web 生产构建与干净提交复验通过，已推送 `captivity-simulator@baf7bda`。
- 副作用边界：本轮未访问 R2、未请求线上模型、未运行线上实测、未部署或重启服务。私有修改仍只在临时分支，尚未提交或推送；`miniapp_static` 中已有本地构建哈希变动，不应随本轮逻辑修复直接发布。

当前状态（2026-07-13 囚禁模拟器第二天逃跑诱导送达修复）：
- 已修复：渡可见投影会为隐私移除 `escape_choice.pending.actor`，旧代码随后误判为“不轮到渡选择”，导致第二天只发送逃跑提示与钥匙位置，却漏掉“尝试逃跑 / 老实待着”两项。现在仅从权威存档读取轮到谁，发送给渡的内容仍严格使用隐私投影。
- 已修复：夜间结算激活第二天逃跑诱导后，会立即在后台同步；转场动画只负责展示，不再决定是否真正送达。必需的特殊事件续接采用最多 3 步白名单，只覆盖夜间结算到逃跑选择、逃跑选择到抓回经过等闭环，普通白天行动仍保持一次同步。
- 已修复：当前待处理为逃跑选择时，前端不再拿昨晚最新事件补进卡片；监控方式也改用中文标签，不再显示 `full / silent` 等内部值。页面只显示今天的逃跑提示、钥匙位置与当前等待状态。
...
- 已完成：只有选择了调教、性交、需要展开的具体玩法或明确 `requires_process` 的段落才要求完整经过；单独选择道具不再自动扩写。该变化仅限 `capture_du`，`captured_by_du` 的原流程和道具触发规则保持不变。
- 已完成：某段含完整经过时，前端先进入经过阅读页；保存后再消费下一段。第三段完成后才进入夜间，渡的夜间自由行动与后续监控仍单独同步。
- 已完成：开源版使用同样的 `capture_assistant` 合批协议和 `{user}` / `{assistant}` 占位符；README 已说明三段同轮返回及 `【过程N】` 规则。
- 已验证：私有版囚禁模拟器规则测试、私有游戏公共测试、MiniApp TypeScript、`import app`、两边 `git diff --check` 通过；开源版 24 项 unittest、Web 构建、TypeScript 和开源审计通过。全部使用本地临时存档或假适配器，未访问 R2、线上后端或模型。
- 发布：开源版已同步；私有版纳入本次 `push重启` 批次。

当前状态（2026-07-13 囚禁模拟器催眠退行抓回路线，仅被渡囚禁）：
- 已完成：仅在 `captured_by_du` 路线的逃跑抓回经过中增加可选 `followup=hypnotic_regression`；渡在同一次抓回回复里提交新规矩、催眠退行分支和完整经过，不增加额外模型轮次。
- 已完成：该分支只保存轻量 `hypnotic_regression` 路线标记，用于 30 天优先结算专属结局「摇篮」和生成渡视角的结局摘要；不保存写作素材清单，也不在之后的动态提示中持续注入路线内容。分支确认后，规则引擎会把之后每次喂食的加料固定为精液；是否向被囚禁方显示仍遵循该次喂食原有的告知方式。
- 已完成：「摇篮」前端正文使用 2026-07-13 单独调用 DeepSeek 生成并由辛玥确认的版本；其中喂精表现为口交后射入口腔并吞咽，不再写成混入奶或食物。
- 边界：`capture_du` 路线的抓回后续菜单不出现催眠退行，规则引擎也拒绝手工提交；MiniApp UI 无改动。开源版使用 `{user}` / `{assistant}` 占位符，并同样只允许 AI 囚禁玩家路线进入。
- 已验证：私有版 `scripts/test_captivity_simulator_game.py`、`scripts/test_private_board_game.py`、相关 `py_compile`、`import app`、`git diff --check` 通过；开源版 21 项 unittest、开源审计和 `git diff --check` 通过。测试仅使用本地临时存档，未调用线上后端、R2 或模型。
- 发布：开源版已同步；私有版纳入本次 `push重启` 批次，并在发布后执行公开健康检查。
...
- 已完成：`services/pixel_home.py::infer_xinyue_state_from_text` 收紧吃饭类状态推断；`吃没吃饭 / 吃饭了吗 / 有没有吃饭` 等询问不再把小玥状态改成厨房，只有 `我去吃饭了 / 我在做饭 / 我点外卖了` 等明确行动才更新。回归测试为 `scripts/test_pixel_home_xinyue_state_inference.py`。
...
当前状态（2026-07-13 囚禁模拟器宠物线深化）：
- 已完成：建立宠物身份时自动启用口头物化、性服务和违令性惩戒三条核心规则；旧存档已有宠物身份时也会在读取时补齐，不需要重开。
- 已完成：新增「口头物化训练 / 性服务训练 / 违令性惩戒」与「宠物物化训诫 / 宠物性惩戒」，都进入具体经过、数值结算、道具推荐和动态事件素材；待处理违令会明确作为后续物化训诫或性惩戒的前因。
- 边界保持：普通行动、喂食、逃跑、监控和礼物机制未改；本轮只深化宠物线，不把所有行动强制改成性内容。
- 已验证：私有版囚禁模拟器全量测试、Python 编译和 MiniApp 生产构建通过；开源版 20 条 Python/MCP/HTTP 测试、占位审计与 Web build 通过。测试只使用本地假存档，没有访问线上存档、R2 或模型。
- 发布边界：改动位于 `/tmp/du-gateway-pet-depth-20260713`，必须从这个隔离 worktree 提交，不要从主脏工作树拼接。

当前状态（2026-07-13 Gemini STT 唱歌分段识别）：
- 已调整：语音转写按时间顺序区分普通说话、无词哼唱和带歌词演唱；歌词清晰时仍保留 `（唱歌）` 发声方式，切回说话时才标 `（恢复说话）`。
...
当前状态（2026-07-14 主动唤醒论坛重复执行与互动时间污染）：
- 根因：`X-DU-PROACTIVE-DECISION` 决策轮保留全量常驻工具，渡可能已在该轮调用 `cli` / `forum_read_feed` / `forum_open_thread`；旧调度只读取最终 `action=forum`，不知道工具已经执行，因而又追加一次论坛执行轮。同时，`X-DU-GATEWAY-WAKEUP` 内部请求误进入跨平台用户活动、最近入口和 QQ 私聊上下文更新。
- 已修：不改工具定义、注入顺序或静态缓存；网关只在主动决策内部响应顶层返回实际执行工具名，调度器检测到论坛工具后把本轮视为已执行并停止追加。没有论坛工具轨迹时，原有单次追加执行仍保留。
