...
Android 只同步播放快照；`services/watch_analysis_source.py` 通过 Bilibili 公开 `view/playurl/player` 接口取得分 P、低清 AVC 主备视频流和音频流。开播前字幕任务先检查 Bilibili 原生字幕；没有原生轨时，`services/watch_subtitles.py` 在配置 `WATCH_SUBDL_API_KEY` 后只使用标准 `original_title + year` 查询一次任意语言 SRT/VTT，按人工 `content_start_ms` 整体平移并写入 `watch_subtitle_assets` 本地表，TTL 24 小时。状态接口只返回语言、版本、格式、条目数、覆盖区间和清洗后的结果，不返回字幕正文、下载地址或 key；重试生成新 `lookup_id`，`/start` 拒绝旧版本。rolling 只读这份已确认资产，不再访问 SubDL；字幕与实际版本冲突时，`watch-v6` 提示词要求 Gemini 以音频和画面为准并保持中文输出。rolling 任务由 `ffmpeg` 提取最多约 140 秒的 16 kHz 单声道 32 kbps MP3，并按 20 秒间隔最多取 8 张图；identify/timeline_prepass 仍只取图。公开解析失败时，可选 `WATCH_ANALYSIS_BILIBILI_COOKIE` 才作为网关专用登录态兜底，设备 WebView Cookie 不上传，公开片源主链不依赖 yt-dlp。`POST .../analysis/samples` 仅为当前画面/诊断备用入口，不负责客户端音频上传。模型默认经 OpenRouter 调用 `google/gemini-2.5-flash`；请求使用 `input_audio + image_url`、严格 JSON schema 且 `reasoning.effort=none`。分析完成后从原始取样帧派生 24 小时低清缩略图，原始 MP3/截图继续及时删除；聊天可按模式临时附一张 2×2 WebP 时间线拼图，拼图不晚于回复抵达位置且不归档。部署除 Python requirements 外要求系统 `ffmpeg`，健康接口会报告取材、知识卡、字幕 provider 和视觉缓存状态。worker 不是 Flask 内线程，代码存在不等于服务已安装或部署。定向验证：`.venv/bin/python scripts/test_watch_together_backend.py` 与 `.venv/bin/python scripts/test_watch_analysis_phase2.py`，后者使用假的 Bilibili API、ffmpeg、字幕、搜索和模型上游且不写 R2。
