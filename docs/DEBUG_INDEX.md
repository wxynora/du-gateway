...
| 动态记忆检索 | `pipeline/pipeline.py`、`services/dynamic_memory_search.py`、`services/dynamic_memory_reranker.py` | 同一次 DS 请求输出 1 条 `RESOLVED` 消歧主查询与 2 条扩展 query，不使用 `MODE`；只在当前消息确有指代、省略或承接时借最近上下文补对象，禁止补未确认因果、交换主语、把明确新动作带回旧话题或生成回复任务。请求显式设置 `thinking.type=enabled`、`max_tokens=2048`、8 秒超时；原始消息继续作为向量保底，`RESOLVED` 与 2 条扩展共同进入向量召回，`RESOLVED` 同时作为 BM25 查询并独立传给 Qwen reranker，召回调试保存 `resolved_query`、`bm25_query` 和两条 `expanded_queries`；定向回归为 `scripts/test_dynamic_memory_context_resolved_query.py`、`scripts/test_dynamic_memory_query_rewrite_payload.py`。cosine、BM25、多 query 支撑度与独立记忆 prior 均归一化到 `0～1`，生命周期权重不再混入相关性分。向量与 BM25 合池后保留到 reranker 配置容量（默认 30 条）交给 Qwen；Qwen 成功返回时以官方 yes/no 概率的 `0.5` 为相关性边界，只让 `relevance_score >= 0.5` 的候选进入 `0.85 rerank + 0.10 recall + 0.05 prior` 排序，低分、未返回和未评估候选均不补位，之后再按 `DYNAMIC_MEMORY_TOP_N`（默认 5）这个上限与动态 token budget 注入，因此可返回 0～5 条。reranker 请求失败仍沿用现有向量/BM25 回退；reranker 前宽候选池继续独立交给动态层 DS，不受最终注入过滤影响。相关性过滤定向回归为 `scripts/test_dynamic_memory_rerank_relevance_filter.py`。多模态当前 user 消息只拼接全部 `type=text` part；图片、音频等非文本 part 不进入关键词、DS、向量、BM25、rerank query 或召回调试 query，定向回归为 `scripts/test_dynamic_memory_recall_multimodal_query.py`。物理淘汰仍是唯一召回生命周期出口，仍保存在动态层中的全部条目都进入候选资格判断，不再按独立 10 天有效期二次过滤 |
