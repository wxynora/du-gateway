...
| 工具使用摘要缓存 | `services/tool_result_cache.py`、`storage/runtime_sqlite.py`、`pipeline/pipeline.py`、`routes/miniapp/reasoning.py` | `tool_result_cache` 继续保存已经处理好的工具摘要事实：`random_imitator_td`、`farm`、`cedareco` 中未混入其他工具的单次或连续工具轮只入独立后台 FIFO，由 `Qwen/Qwen3-8B` 融合为一条后写本地 SQLite；其他工具轮仍同步按工具清洗。24 小时 TTL，按实际注入字符计数，超过 3000 字符时删除最早完整记录直至不高于 2000。`prompt_tool_generation` 另存每个窗口的 stable 快照、当前 4 轮 Hot、已封存 chunk ID 与 recent tool batches：工具摘要写入后追加到 Hot；对应 4 轮近期记忆成功落盘时，把非空 Hot 合成一个 `〖本段工具摘要〗` physical system block 并清空 Hot；封存后的批次不再受原始 TTL/裁剪影响，尚未封存的 Hot 会同步移除已被原始缓存淘汰的项；第 8 组先封 T8，再随第 32 轮 generation rollover 用当前有效原始摘要重建 stable 快照并清空上一代 T 批次，全程不再次调用模型。旧表行标记为 legacy，保留现有 Hot，直到它为空或按旧 2000 字符规则并入 frozen 后自动切入 rolling，不追溯旧 Hot 的 4 轮归属。思维链接口根据每轮已归档的 `static_breakdown` 汇总 stable、recent tool batch 与 Hot 字符数，并直接返回同一轮保存的 `channel`，不读取页面刷新时的全局现值；旧轮次没有通道快照时返回空字符串 |
