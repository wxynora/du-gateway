...
| 身体状态四轮评估 | `services/du_body_evaluator.py`、`storage/du_body_eval_store.py`、`services/pixel_home.py` | 真实归档轮次独立进入 SQLite pending，每 4 轮或最旧等待 30 分钟时由 DS 逐轮输出 delta；apply 使用稳定幂等键并记录 before/delta/after 审计，失败保留重试，进程重启按 lease 接手；不改变动态记忆、近期总结或压缩移位计数，旧动态层 BODY 默认只解析不应用 |
...
- 一起看聊天仍走同一 SumiTalk job。请求顶层附带 `watch_session_id` 和完整 `watch_snapshot` 后，`services/watch_context.py` 按消息发送时 playhead 注入动态上下文；`needs_summary` 只读取不晚于消息快照的最近剧情检查点。未来两分钟片段只能生成隐藏弹幕，`services/watch_action_flow.py` 在流式与非流式链路剥离原始块并发出 `watch_danmaku_action`。事件不使用主动消息 channel，seek 后旧 epoch 动作失效。
...
- 工具摘要表：`tool_result_cache`；身体状态评估表：`du_body_eval_pending`、`du_body_eval_audit`；一起看运行表：`watch_sessions`、`watch_timeline_sections`、`watch_plot_chunks`、`watch_risk_events`、`watch_risk_feedback`、`watch_analysis_samples`、`watch_analysis_jobs`、`watch_timeline_fingerprints`、`watch_story_checkpoints`（均在同一运行 SQLite；非 R2）
...
- 一起看：`routes/miniapp/watch.py`、`storage/watch_runtime_store.py`、`storage/watch_analysis_store.py`、`services/watch_analysis.py`、`services/watch_analysis_source.py`、`services/watch_analysis_samples.py`；已实现会话/播放快照、模式、时间轴、网关自动取材计划、备用样本上传、分析任务查询、队列诊断、剧情检查点、高能反馈和状态查询，接口统一位于 `/miniapp-api/watch/*`
...
一起看 Phase 2 分析 worker 入口为 `scripts/run_watch_analysis_worker.py`，安装脚本为 `scripts/install_watch_analysis_worker_service.sh`。Android 只同步播放快照；worker 扫描活跃会话并创建 `backend_source` 任务，`services/watch_analysis_source.py` 通过 Bilibili 公开 `view/playurl/player` 接口取得分 P、低清 AVC 主备视频流和可用字幕，再由 `ffmpeg` 按计划媒体时间最多 3 路并发取帧。公开请求失败或公开字幕为空时，可选的 `WATCH_ANALYSIS_BILIBILI_COOKIE` 才会作为网关专用登录态兜底；设备 WebView Cookie 不上传，诊断只返回兜底是否配置，公开片源主链不依赖 yt-dlp。`POST .../analysis/samples` 仅为当前画面/诊断备用入口。模型默认经 OpenRouter 调用 `google/gemini-2.5-flash`，`watch-v2` system prompt 要求把事实、角色神情和有依据的少量氛围组织成连贯剧情文字，禁止流水账、主观影评和无证据心理脑补；请求使用严格 JSON schema 且 `reasoning.effort=none`。临时截图只落本地并在完成、取消、永久失败或会话过期时删除。部署除 Python requirements 外要求系统 `ffmpeg`，健康接口会报告取材源就绪状态。worker 不是 Flask 内线程，代码存在不等于服务已安装或部署。定向验证：`.venv/bin/python scripts/test_watch_together_backend.py` 与 `.venv/bin/python scripts/test_watch_analysis_phase2.py`，后者使用假的 Bilibili API、ffmpeg 结果和模型上游且不写 R2。
