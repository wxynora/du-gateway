...
| 能力 | 代码入口 | 当前边界 |
| --- | --- | --- |
| 请求接收、流式响应、工具循环、隐藏标记 | `routes/chat.py` | 所有支持入口走统一主链路；流式与非流式工具循环共用 `config.py::TOOL_MAX_ROUNDS`，默认最多继续 15 轮，显式环境变量仍可覆盖 |
...
| Pipeline 组装与归档 | `pipeline/prompt_layout.py`、`pipeline/prompt_content.py`、`pipeline/context_injections.py`、`pipeline/recent_context.py`、`pipeline/request_prepare.py`、`pipeline/memory_recall.py`、`pipeline/memory_evolution.py`、`pipeline/archive.py`、`pipeline/post_archive.py`、`pipeline/request_context.py`、`pipeline/pipeline.py`、`routes/chat.py::_prepare_chat_request` | `pipeline.py` 只保留兼容导出；提示词布局、内容注入、近期上下文、请求准备、记忆召回/演化、归档和归档后任务按权限分开。每次请求在进入准备阶段前一次性冻结 headers、active upstream URL 与 Anthropic format，后续组装只读取 `PipelineRequestContext` 快照，不再二次读取 Flask request 或重新查询 active upstream |
...
| 提示词管理 | `services/prompt_manager.py`、`storage/r2_store.py` | App 可编辑的静态 Prompt 分区统一从这里管理；目录首项“世界层级”是整个 system 序列的固定静态首槽位，分区标题只供管理页展示、不进入注入正文；Codex OAuth 专用 Prompt 仅在对应上游命中时作为独立 system 置于世界层级之后、核心 Prompt 之前；对话模式只保留“现实模式”和“Real 模式”两个可编辑区。Claude Opus 4.6 的现实物理层请求使用 `pipeline.OPUS46_REALITY_MESSAGE_PROMPT` 后端固定专用正文，不读取共享“现实模式”section；GPT、GLM、DeepSeek、其他非 Claude 与其他 Claude 模型继续读取现有共享“现实模式”正文，Real 模式对所有模型仍读取共享“Real 模式”正文。QQ/TG/SumiTalk 按请求模式互斥选择现实或 Real；现实模式正文运行时替换 `{channel}` 和 `{sticker_tags}`，因此渠道名与当前表情包英文标签不会固化进保存正文；QQ 使用现有 `X-Reply-Target` 只区分模型可见显示名：`qq_group_mention` 为 `QQ群聊`，其余 QQ 为 `QQ私聊`，内部 `reply_channel` 仍保持 `qq`；原生 Prompt 管理页无需新增 section 或适配；目录末尾固定提供 `custom_static_system_1..5` 五个允许为空的自定义 System 槽位，空槽不注入，非空槽按编号进入固定静态尾部；详情最多返回当前分区最新 3 个备份，配置写成功后按 `created_at` 清理到最新 3 个，写失败不删旧备份，回滚复用同一保存与保留链路 |
| 入口风格 | `services/entry_style_prompt.py` | QQ、TG、SumiTalk 使用“现实模式 / Real 模式”，微信不再注入旧入口风格；只保留小爱音箱专用入口规则 |
| 语音台词规范 | `services/voice_line_prompt.py`、`pipeline/pipeline.py` | QQ、TG 与 SumiTalk 请求统一把该规范作为 BP2 前最后一个固定静态块；QQ 私聊与群聊的显示名细分不改变语音规范注入资格 |
| 语音转写后处理 | `services/stt.py` | OpenRouter 默认模型为 `google/gemini-3.7-flash`；每个实际候选模型独立决定 provider 路由，只有 `google/*` 候选固定 `google-vertex/global`，配置为非 Google 的 fallback 不继承 Vertex 限制。Gemini/OpenRouter 与 Deepgram 在统一返回边界压缩同一个非词汇填充音的超长连续重复：至少 5 次时保留 3 次并以中文省略号分隔；短重复、混合发声和有意义的词语重复保持原文 |
...
| 网页搜索与正文压缩 | `services/web_search_tools.py`、`services/worker_models.py`、`config.py` | Tavily 搜索、页面抓取、HTML/乱码与常见页脚噪音清洗保持原链路；抓取后的相关正文通过 structured 岗位使用硅基流动免费 `Qwen/Qwen3-8B` 非思考模式压缩，并由岗位层复用现有硅基密钥，仍保留既有 6000 字输入、900 tokens 输出、25 秒超时、Prompt 和失败状态 |
| 后台模型岗位表 | `services/worker_models.py` | `translate / structured / background_reasoning / ocr / asr / rerank` 统一返回 provider、协议、URL、密钥和 model；同协议换模优先覆盖对应 `WORKER_*_MODEL`，换 provider 时同处覆盖 provider/URL/key。translate 当前为硅基混元 MT；structured 当前为硅基 Qwen3-8B，只接网页压缩和工具摘要；background_reasoning 当前为硅基流动 `deepseek-ai/DeepSeek-V4-Flash`，接动态层 new / merge / skip、身体状态评估、中期记忆、记忆维护去重、人工记忆重写、近期总结和文游 GM，以上链路不再回落 DeepSeek 官方 V4 Flash；ocr 当前通过 Google AI Studio OpenAI compatibility 使用 `gemini-3.5-flash-lite`，仅从 `WORKER_OCR_*` 读取配置；asr 保留 Deepgram/OpenRouter 双适配与既有主备顺序，rerank 沿用硅基 Qwen3-Reranker-8B。一起看知识卡仍保持现有硅基 DS V4 Flash 独立链；查询改写的 `deepseek-v4-pro` 与主模型链不属于 background_reasoning；新协议仍须新增对应适配器，不把协议差异伪装成只换 model ID |
| 工具使用摘要缓存 | `services/tool_result_cache.py`、`storage/runtime_sqlite.py`、`pipeline/pipeline.py`、`routes/miniapp/reasoning.py` | `tool_result_cache` 继续保存已经处理好的全局工具摘要事实：`random_imitator_td`、`farm`、`cedareco` 中未混入其他工具的单次或连续工具轮只入独立后台 FIFO，由 `Qwen/Qwen3-8B` 融合为一条后写本地 SQLite；其他工具轮仍同步按工具清洗。24 小时 TTL，按实际注入字符计数，超过 3000 字符时删除最早完整记录直至不高于 2000。`prompt_tool_generation` 另存每个窗口的 stable 快照、当前 4 轮 Hot、已封存 chunk ID 与 recent tool batches：rolling 工具摘要只按自身 `window_id` 追加到同窗口 Hot，不进入其他窗口；对应窗口的 4 轮近期记忆成功落盘时，只把该组绑定的工具项合成一个 `〖本段工具摘要〗` physical system block 并从该窗口 Hot 移除，其他窗口不受影响。封存后的批次不再受原始 TTL/裁剪影响，尚未封存的 Hot 会同步移除已被原始缓存淘汰的项；第 8 组先封 T8，再随第 32 轮 generation rollover 从全局当前有效 `tool_result_cache` 重建该窗口 stable 快照并清空该窗口上一代 T 批次，全程不再次调用模型。旧表行标记为 legacy，保留现有全局 Hot，直到它为空或按旧 2000 字符规则并入 frozen 后自动切入 rolling，不追溯旧 Hot 的 4 轮归属。思维链接口根据每轮已归档的 `static_breakdown` 汇总 stable、recent tool batch 与 Hot 字符数，并直接返回同一轮保存的 `channel`，不读取页面刷新时的全局现值；旧轮次没有通道快照时返回空字符串 |
...
| Claude thinking 连续性 | `services/claude_thinking_carryover.py` | 普通对话回传 opaque signature，不回灌转写 thinking 文本；只有实际返回 carryover 的上一轮才从 last4 排除，未返回 carryover 时不丢上一轮正文 |
...
| Prompt Cache 诊断 | `services/prompt_cache_debug.py` | 记录静态/动态构成与上游 usage 元数据；语音规范显示为独立固定静态块并标记 BP2，模式与 Thinking 按实际旧格式动态块或中途 system 形态显示，长期记忆与中期记忆分别显示 |
...
当前 Claude 缓存前缀顺序固定为：tools → BP1（1h）→ 固定静态子块（system 首项恒为世界层级；Codex OAuth 专用 Prompt 命中时位于核心 Prompt 之前；尾部为五个自定义静态 System 槽位中的非空项，按 1→5）→ 独立语音规范 → BP2（1h）→ 小爱入口风格（命中时）→ generation 冻结块（当前代 stable 工具摘要与 `older + slightly` 稳定近期记忆合并为同一 system）→ 按 4 轮批次交错排列的 `R1/T1、R2/T2……`，无工具的批次只有 R → BP3（最后一个已封存 T，若本批无 T 则为最后一个 R，1h；无 recent 时回退 generation 冻结块）→ 当前 chunk window 的 Hot 工具摘要，不设独立断点 → “你的日常”常驻动态独立块 → 其他常驻动态 → 临时动态。迁移中的 legacy 工具生命周期在旧 Hot 完成原有并入前暂时保留旧 BP4；切到 rolling 后第四个显式断点不使用。Opus 4.6/4.7、Sonnet 5 与非 Claude 模型继续在这一动态尾部按“草稿提醒 → Thinking 规范 → 当前模式 → last4 → 对话消息”排列；其中只有 Opus 4.6 的现实物理层“当前模式”换用后端固定专用正文，其他模型沿用现有共享版本，槽位与缓存标记均不变。支持 Anthropic 中途 system 的模型不在前缀里放模式或 Thinking，而是在当前 user/tool_result 后追加一条合并了当前模式与 Thinking 规范的 `role=system` message。当前显式支持 Opus 4.8/5、Fable 5、Mythos 5；Opus 4.6/4.7、Sonnet 5 和其他模型回退旧格式。春梦链路 `spring_dream` / `random_spring_dream` / `post_spring_dream` 不加载“现实模式 / Real 模式”，春梦后唤醒按现实请求恢复相应模式。“你的日常”在生产点显式携带 `__dynamic__=True`，位于所有缓存断点之后；常驻动态、临时动态、模式、Thinking 和 last4 保持独立语义，不会拼进固定静态前缀。play 小纸条仍由 `services/pixel_home.py` 生成，内容与触发条件沿用原逻辑，只位于临时动态位置。
...
`step_inject_tool_result_cache()` 使用 `pipeline/pipeline.py` 的唯一 `_SYSTEM_PROMPT_REGION_ORDER` 和 `_SYSTEM_PROMPT_CACHE_GROUPS`：固定静态、独立语音规范、入口风格、generation 冻结块、按 chunk ID 交错的 recent memory/tool batches、hot、日常常驻动态、其他常驻动态、临时动态、草稿提醒、独立 Thinking、旧格式模式、last4；中途 system 模式不参与前缀重排。语音规范是 BP2 前最后一个固定静态块；stable 工具摘要与稳定近期记忆在当前 generation 内同时冻结并合成一条 system；每个 R 保持独立 block，同一 4 轮内多个工具摘要只合成一个 T block，Hot 保持在主断点之后。近期摘要 state 为 version 3：每 4 轮只 append 一条 recent，每 8 轮只写最多 `2 + 2` 条 `next_*` staging，每 32 轮才提交层级迁移、淘汰与 generation 切换；第 8 个 pending 只占位、不换代，补写成正式 R8 后先封旧工具代的 T8，再切 memory/tool generation；任一 4 轮总结失败后不继续处理后面的缺口，也不提前封 Hot。v3 normalize/render 不执行 `recent[-8:]`，`recent=8` 只表示 generation commit 后目标容量。`storage/r2_context_store.py::save_summary_chunks()` 保留 v3 版本；曾被旧 writer 误标成 version 2、但带完整 generation contract 的 state 会按 v3 原样恢复，不再重复走 legacy trim。工具原始结果仍在 SQLite `tool_result_cache` 按既有 TTL/容量管理；`prompt_tool_generation` 持久化 stable、Hot、封存 chunk IDs 和 recent batches，异步工具摘要以实际完成写入的事务先后决定进入旧 T 还是下一组 Hot。Python Anthropic 转换、Claude OAuth proxy 与 Pioneer 兼容转换都按显式 marker 把 BP3 放在最后一个已封存 R/T block；rolling Hot 不设断点，迁移期 legacy Hot 保留旧断点，内部 marker 在实际出站前删除；支持中途 system 的消息保留在 Anthropic `messages`，不提升到顶层 `system`；网关只向本地 OAuth proxy 与原生 Anthropic 转换链透传 body 级 cache layout，代理在实际 Anthropic POST 前记录 window/generation/update/recent tool batch/hot 与实际断点，并删除该内部字段。OAuth 非流式和流式 usage 都保留上游存在时的 `cache_creation.ephemeral_1h_input_tokens` 与 `ephemeral_5m_input_tokens`。
...
- 异步游戏工具摘要入 FIFO 前按 `window_id + 有序 tool_call_id` 登记 pending 任务；近期总结从对应 4 轮归档的 assistant `tool_calls` 还原同一任务键，并在调用 DS 生成 R 之前等待这些任务完成写入或走完既有确定性回退。每个 R chunk 同时保存由本组工具调用推导出的精确缓存 item ID；封 T 只消费这些 ID，DS 生成期间后来写入的下一组工具仍留在 Hot。只等待、只封存本组真实引用的任务，不等待下一组或其他窗口；R 与对应 T 在同一次 4 轮封包中进入 recent。第 32 轮 rollover 会把未绑定的剩余 Hot 带入新 generation，并从新 stable snapshot 排除这些后续项。
- generation rollover 由 `services/tool_result_cache.py::prompt_generation_contents()` 在一个 SQLite `BEGIN IMMEDIATE` 事务内完成：补封上一代尚缺的 T8、按当前有效原始工具摘要建立新 stable、切换 generation 并清空上一代 recent batches / Hot。事务开始前已写入的工具进入上一代 T8 和新 stable；事务开始后竞争写入的工具等待提交，再进入新一代 Hot，不会被 stable 误收。
- 同一 `window_id` 的 4 轮近期总结任务在 `pipeline/pipeline.py` 内按窗口串行；后触发任务取得锁后重新读取最新 summary / chunks，不能与前一任务并发覆盖 R2 或把工具 T 配到错误的 R。不同窗口仍可并行。
- Python Anthropic、Claude OAuth 与 Pioneer 的 BP3 都以最后一个已封存 R/T 为准；没有 R/T 时回退到合并的 frozen 工具摘要与稳定近期记忆块，Hot 不设断点。Prompt Cache 调试快照记录 `tool_lifecycle_mode`；rolling 统计的 `max_chars` 返回 `null`，表示没有旧式 5000 字符总上限，legacy 历史轮次仍保留 5000 旧口径。
...
- Doorbell Bell 首户 adapter 位于 `scripts/doorbell_bell_injector.py`。当前部署版本仍只接受 version 1、历史 `mailbox_unread` reason 与当时批准的固定提示；该原因已从 Doorbell 服务端生产者中移除，遗留 pending 只会收到 cancel，因此 adapter 目前不会获得新的社区业务 wake。信件标题／正文、Bell token 和额外字段全部拒绝。`wake_id` 映射为 `client_request_id=doorbell-bell:<wake_id>` 后调用既有 `build_sumitalk_chat_job_payload()`；同 wake 重放只复用非错误／非取消的原持久任务。未来接入任一已批准社区生产者前，必须另行更新并审阅该家庭 adapter 的 reason／message 白名单；不能把旧邮箱提示复用于新业务。系统通知修正曾随 main `8d654851` 发布，Bell 保持 active/enabled；历史验收没有手工生成通知、调用真实模型或发送消息。
...
普通随机唤醒的主决策与随机冲浪后二次决策均提供论坛选项；主决策渲染保留托管模板中的 `逛论坛`/`forum` 候选。选择 `forum` 后追加独立执行轮，使用统一 `galatea_garden` 工具先以 `action=list_threads` 浏览，再按需以 `action=get_thread` 打开帖子；旧 `forum_read_feed` / `forum_open_thread` 不重新注入。 初始决策和随机冲浪后二次决策的现行控制协议均为单个 `<decision>` XML；`action/reason/message/game/channel` 分字段输出，其中自由文本使用 CDATA，解析失败的疑似控制结构不会原样外发。
...
QQ 群通过 `QQ_GROUP_ID` 绑定唯一群，当前为 `515831305`：OneBot connector 在解析正文、记录历史或调用网关前忽略其他群，`/push/group` 同样拒绝非绑定目标；网关只记录并读取该群的活动，旧 R2 群活动保留但不会进入上下文。绑定群上下文按发言人区分，不把群友内容当成小玥说的；入口消息仍进入统一聊天主链路。群聊上下文只允许后端随机主动决策、半小时硬触发、日历事件和系统闹钟使用；小家/身体/道具、延迟续话、屏幕检查、日记、游戏等其他后端事件与普通聊天均不注入。群聊文字上下文保持最近 20 条、单条 180 字；归档回读会识别 `Q编号` 发言格式并移除仅供当前回复使用的编号/引用说明，避免与 last4 重复。主动唤醒的群图只有在晚于最近一次主动消息且仍在一小时有效期内时才参与，上下文最多携带最近 5 张图片：OneBot 入口把 QQ 临时图片 URL 转成 base64，网关继续复用统一图片压缩；每张图片保留原群消息的发送者与时间，并在 `user` 多模态内容中紧邻图片前标注，避免把群友图片误认为小玥发送；不为这类上下文图生成图片描述；单张图片获取或压缩失败时只把该图回退为 `【图片】`，不影响其他图片和本轮唤醒。上下文存在时会同时告知渡可在回复正文开头使用 `[QQ_GROUP]`；随机主动决策的 XML 把标记写在 `<message>` 的 CDATA 正文开头。网关只从该上下文保存的来源群号生成后端投递元数据，模型不能选择群号；标记在归档和投递前剥离，经 OneBot connector `/push/group` 发回绑定群，失败则回退原唤醒渠道。

QQ 群内直接 @ 渡或引用渡本人消息的回复由 `connectors/qq_onebot/src/main.js` 控制：真实 @ 仍只认目标为机器人账号的 OneBot `at` 段；无 @ 的引用会通过既有 `get_msg` 拉取被引消息，只有发送者账号等于当前机器人时才触发渡，引用其他群友不触发。本轮可见群消息按顺序标为临时 `Q1/Q2…`；渡可以在每条独立出站文字的开头分别写 `[QQ_REPLY:Q编号]` 或 `[QQ_AT:Q编号]`，同一轮允许先引用一人、后续分段再真实 @ 另一人；每个标记只作用于紧随其后的一个实际出站消息，不写标记则普通回复，直接写 `@名字` 仍只是文字。connector 只接受本轮真实编号，QQ 号不进入模型上下文，无效标记在发送前剥离；标记后只有表情或语音时仍由下一条实际出站消息消费。引用渡触发与真实 @ 共用黑名单、重复事件和连续次数限制：每个群最多连续处理 10 次，第 11 次起在调用网关前停止回复；相邻两次 @/引用渡的间隔不超过默认 5 分钟的 `QQ_GROUP_MENTION_CONTINUITY_GAP_MS` 则继续累计，普通群消息不重置；被拦截的触发也会刷新最后触发时间，只有下一次 @/引用距上一次超过该间隔时才从 1 重新计数。QQ 私聊与主动 `/push/group` 保持原行为。
...
| 思维链最新记录与翻译 | `routes/miniapp/reasoning.py`、`storage/conversation_sqlite_store.py::get_latest_rounds` | `GET /miniapp-api/reasoning/latest` 直接从 SQLite `conversation_rounds` 按 `timestamp DESC` 跨窗口取最新轮次（默认 10 条），只为这批轮次组装思维链、记忆召回、缓存、费用和工具记录；不再枚举最近窗口或逐窗口扫描 80 轮，响应字段与原生 App 协议不变。`POST /miniapp-api/reasoning/translate` 复用现有硅基流动 host 与密钥解析，使用当前免费翻译专用模型 `tencent/Hunyuan-MT-7B`；原高保真翻译 Prompt、6000 字分段、动态输出额度、温度、90 秒超时与接口响应保持不变 |
...
| 动态层判断 | `services/dynamic_layer_ds.py`、`services/memory_merge_rules.py` | 当前轮先按同一具体事项拆分，并用 `ITEM: n` 分块分别产生原有 new / merge / skip 决策；主体、对象、关系/行为和具体内容不同的事项不得混进同一块，每条旧记忆一轮最多由一个块更新，`pipeline/pipeline.py::_step_dynamic_layer_evolve` 按块逐条应用。候选直接复用本轮主聊天 reranker 前宽候选池，动态层不再构造独立检索 query 或执行额外 embedding；给 background_reasoning 岗位模型的 Prompt 与审计预览保留 role/content，并把 reasoning 与 thinking_blocks 中的思路正文去重、归一为单一 thinking 字段，用于判断渡的感受、误解和认知变化；signature、request id 等协议元数据不传。动态层与人工记忆重写共用同一份 merge / 迭代规则；merge 必须核对主体、对象、关系/行为和具体事项，只有关键词、标签、房间或宽泛语义相近时不得融合，名字羞耻症与渡的名字记忆是明确反例；只有当前内容明确表示这是不同日期发生的另一次一次性事件时，即使人物和行为相同也禁止普通 merge，有独立记忆价值时用 new，否则 skip；多次独立事件已形成稳定习惯、XP、偏好或边界观察时只可使用待审的 habit_generalization；其余情况仍按原有的同一具体事项标准判断，同一过去事件不能仅因本轮正在谈论就改成现在，明确纠正事件时间时则可更新；merge 返回正文会完整替换旧正文，因此以选中旧记忆为底稿再融合本轮增量，且不受 new 的 35-70/必要时 90 字建议约束；未冲突内容继续合并同类项，重复表述去重、互补信息融合、关键事实与感受保留，不得为压短删除；冲突部分不得无痕覆盖，其中被纠正的主观判断降回“当时的理解或误解”而不再作为当前事实，当时真实发生的感受与经历仍保留，本轮真实新增的具体情绪、动作或态度也必须保留；认知变化已自然成立时直接收住，不追加抽象点题；共享规则内含辛玥确认的空壳/底座三项 merge 示例并注明只学信息处理、不照抄句式；禁止固定时间句式、反省清单及无依据编造；merge 输出 `consolidate/correction/invalidate/supersede/temporal_update/habit_generalization` 六类规范原因，明确记反/事实纠错使用 correction，旧状态完成或进入下一阶段使用 temporal_update；只有多次独立重复且正文已归纳共性时使用 `habit_generalization` 并转人工待审，仍写成“上次/这次”事件串联时禁止使用；普通同一事项补充仍自动 merge。动态记忆继续保留原有“事实 + 情绪”要求，只额外禁止“又 X 又 Y”情绪句式，不限制具体情绪词；缺失或非法原因进入现有重写机制，动态层血缘与待审候选保留原因字段；当前不按原因自动删除或淘汰记忆；通过 background_reasoning 岗位走硅基流动 `deepseek-ai/DeepSeek-V4-Flash` 并沿用模型默认 thinking，单轮多事项、批处理和归档批处理的总 `max_tokens` 均保持 2048 |
| 动态层异步响应等待 | `services/dynamic_layer_ds.py` | 单轮、批处理和归档批处理均保留原连接超时：单轮 60 秒，两个批处理 120 秒；三者响应读取上限统一为 180 秒，完整响应在此范围内返回后进入既有解析与落库，避免原单轮 60 秒过早丢失慢结果，也避免死连接无限占用后台线程。网络异常仍走原有 `api_error -> skip`，不新增重试或 fallback |
| 卧室 merge 时间边界 | `services/dynamic_layer_ds.py`、`pipeline/pipeline.py::_apply_one_decision` | 在线候选使用内容 `updated_at`、缺失时回退 `created_at`，结合当前轮北京时间生成 `content_updated_at` 与 `merge_gap_hours`；`last_mentioned` 不作为事件连续性时间。gap 不超过 24 小时只表示可能连续，仍须是同一段具体互动；gap 超过 24 小时时视为独立互动，卧室普通 consolidate/temporal_update/supersede/invalidate 由现有动态层模型重写链改为独立 new/skip，不会自动 merge。跨时段只允许明确纠正同一条过去记忆的 correction，或多次独立事件已形成 XP、稳定偏好/边界观察的 habit_generalization；普通独立事件与 habit 例外已在 Prompt 中明确分流，habit 可压缩重复证据为共性，但不得抹掉共性无法涵盖且仍有独立意义的旧内容。habit 沿用原动态待审，跨时段卧室 correction 也进入同一 pending_merge 待审，不直接覆盖。内部 `bedroom_cross_day` 由动态层候选层统一计算并随决策传给应用层，应用层不重复维护阈值。该边界只限制未来 merge，不删除、改写或自动拆分现有历史卧室记忆 |
| 动态层候选与结果校验 | `services/dynamic_layer_ds.py`、`pipeline/pipeline.py` | 在线候选检索无命中或异常时保持空列表，不再拿最近 10 条无关记忆替代；空候选 Prompt 明确只允许 `new/skip`。每个 merge 块必须在现有重试阶段返回可解析到本轮候选的 ref，缺失、非法或同一轮重复更新同一 ref 时重写全部事项块，连续失败按 skip，绝不降级成 new。new/merge 正文照抄本轮原话时由动态层模型定向重写，连续失败按 skip，网关不再用 18 字片段拼“形成共识”等通用便签。TAG 必须是客厅/书房/图书馆/卧室之一，缺失或非法时由动态层模型重写，连续失败按 skip；应用层只做枚举校验，不再因原文出现“私密/亲密/性暗示”等关键词强改卧室。历史单块固定标签输出仍兼容解析；多事项审计保存各块结果及 action 计数。多轮批处理与归档批处理仍保持原有每轮一个 new/skip 决策、tag/照抄校验及失败边界，不随在线多事项协议改变 |
| Merge 审核学习 | `services/dynamic_layer_ds.py`、`services/dynamic_memory_provenance.py`、`services/memory_rewrite.py`、`pipeline/pipeline.py`、`storage/r2_store.py` | 人工直接通过、编辑后通过和拒绝待审 merge 时，分别以 `merge_review_approved / edited / rejected` 写入既有 provenance SQLite；记录正式旧正文、模型候选、最终正文、merge_reason 和原 pending 的窗口/轮次，不把审核结果写回 R2 schema。动态层只为本轮已经召回的候选查询其各自最新审核结果并以 `review_feedback` 交给同一个动态层模型：直接通过是正例，编辑通过以最终正文为准，拒绝是反例；网关只按候选记忆 ID 取例子，不另调模型或自行判断 merge 语义。带 `pending_merge` 的动态/核心候选标为 `merge_locked`，Prompt、结构校验、应用层和 stage 写入均禁止再次更新；完全相同的重复 stage 幂等成功，不同候选不得覆盖。动态 pending 的展示原因按真实 merge_reason 生成，不再把 correction 等类型写成 habit。`DYNAMIC_MEMORY_REVIEW_ALL_MERGES=1` 时普通动态 merge 也只暂存待审，默认关闭以保持未显式启用时的现行自动 merge；habit、核心 merge 和跨时段卧室 correction 继续按原规则始终待审 |
...
| 动态层候选复用 | `pipeline/pipeline.py::step_inject_dynamic_memory`、`routes/chat.py`、`services/chat_archive_helpers.py`、`services/dynamic_layer_ds.py::call_dynamic_layer_ds` | 主聊天前置召回保存 reranker 前宽候选池的有序 ID，并沿流式、SumiTalk 流式后台队列、普通同步存档和非流式后台慢任务传给动态层。动态层执行时按 ID 从当前动态记忆和核心 pending 重新取正文，直接把这批宽候选交给动态层模型；不再执行原 top10 纯向量二次召回，也不读取调试事件作为数据源。前置召回跳过或无命中时使用空候选，Prompt 只允许 new/skip，不另行 fallback 或 embedding。候选池容量、融合排序和 reranker 行为均沿用主聊天现有实现 |
...
| 近期总结 | `services/deepseek_summary.py`、`pipeline/pipeline.py` | 每 4 轮生成一个近期记忆块，每 8 轮在同一次 DS 更新中执行分层压缩迁移；最近 / 稍早 / 更早最多分别保留 8 / 10 / 7 个，共 25 个，填充期只迁移为下个压缩点预留位置所需的数量，填满后每个压缩点依次迁移 2 个并淘汰最旧 2 个；生成新段时把时间上紧邻的上一条正式总结正文作为只读衔接上下文交给 DS，但 `new_chunk` 仍只能总结本轮 4 轮素材，补填 pending 槽位时只取该槽位之前的正式块；生成/待补结果保存前与聊天注入前不再套总 token budget 二次裁剪；已有轮次元数据时会从首个已知分组起补全部缺口，不再只回看 6 组或每轮只处理最近 2 个缺口；某组生成失败并写入 pending 后，本次任务立即停止，不越过该缺口继续处理后组，也不封存对应工具 Hot；第 8 个 pending 只保留旧 generation，待正文真实补写完成且没有其他 pending 时才执行 generation commit，并先封存对应 T8。请求统一通过 background_reasoning 岗位走硅基流动 `deepseek-ai/DeepSeek-V4-Flash`，原有 3 次总尝试只重试同一硅基上游，不再切回 `DEEPSEEK_*` 官方地址；近期记忆总结继续显式关闭 thinking，不包含 Notion 小本本分支。现行渡日常维护仍由主网关当前 active model 生成；旧 `fetch_daily_whisper_from_summary` 没有调用者，本次未修改；查询改写 V4 Pro 与主模型链不受影响 |
...
- 日常面板与小家状态：`routes/miniapp/dashboard.py`；普通聊天中的小玥状态推断由 `services/pixel_home.py::infer_xinyue_state_from_text` 负责。该入口使用 `chinese_calendar.is_workday` 按中国法定节假日与调休安排判断，北京时间真实工作日 08:00–17:00（不含 17:00）对所有聊天状态意图返回无更新，完整保留已有位置和活动；在单位吃饭不会切到厨房，写代码、洗澡、睡觉或玩手机等文本也不会覆盖“外出上班”状态。法定休息日和其他时段沿用原判定，吃饭或洗澡开始时进入厨房或浴室，明确说吃完、吃饱、洗完或洗好后自动回到客厅休息；不使用 weekday 兜底。手动小家设置、MiniApp 事件及渡的状态链不在此锁定范围。小家固定静态规则与临时动态事件的模型可见住所名均为“星落小屋”；Real 模式读取其独立可编辑正文，状态字段与现有协议不变
...
- 媒体、贴纸与日志：`routes/miniapp/media.py`、`routes/miniapp/stickers.py`、`routes/miniapp/logs.py`。表情包分类以 R2 `stickers/meta.json` 为真相源，`storage/r2_sticker_store.py::save_stickers_meta` 只在 R2 写成功后原子更新 `data/sticker_tag_keys.json` 同机共享缓存；`services/sticker_tags.py` 让各 Python 进程按缓存文件版本读取，缓存不存在或显式 `refresh` 时才回源 R2。QQ/TG/SumiTalk 共用“现实模式”正文中的 `{sticker_tags}` 占位符，注入前替换为共享缓存里的当前英文标签；兼容旧保存正文中已有的“当前全部可用英文代号”行并原位刷新，其他人工编辑保持不变。QQ connector 的 `/miniapp-api/stickers/tags-public` 仍按原 45 秒本地 TTL 获取同一列表。
- 音乐：`routes/miniapp/music_bgm.py`、`routes/miniapp/music_netease.py`、`services/music_melody_analyzer.py`；旋律分析默认模型为 `google/gemini-3.7-flash`，只有当前实际调用的 `google/*` 候选固定 `google-vertex/global`，非 Google fallback 不继承 Vertex 限制。
...
文游玩家工具默认不进入聊天工具集。App 负责管理“无限流游戏模式”这个全局开关；开启后，统一聊天入口都会注入 `buy_item`、`roll_gacha`、`inventory_action`、`use_item`、`transfer`，关闭后所有入口都不注入。文游 GM 通过 background_reasoning 岗位使用硅基流动 `deepseek-ai/DeepSeek-V4-Flash`，保留模型默认 thinking 和总 `max_tokens=8192`；旧 `WENYOU_DS_MODEL` 配置已删除。
...
截至 2026-08-15，生产 VPS 的 `du-main-poll.timer` 已停用，状态为 `disabled`、`inactive`，不会再每五分钟自动拉取并发布 `origin/main`。`du-main-poll.service`、`/usr/local/sbin/du-main-poll` 与 `/usr/local/sbin/du-deploy-main` 保留，用于明确授权后的手动发布或日后显式恢复自动轮询；不得把脚本仍存在误判为自动部署仍在运行。

当前发布应先核对消息处理与投递为空闲，再从本机通过 `ssh du-gateway` 手动调用生产正式入口；关闭自动轮询不影响现有网关、worker 或手动发布脚本，也不需要重启网关。只有辛玥明确要求恢复自动发布时，才允许重新启用 `du-main-poll.timer`。
