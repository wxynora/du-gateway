...
| 动态记忆检索 | `pipeline/pipeline.py`、`services/dynamic_memory_search.py`、`services/dynamic_memory_reranker.py` | 当前消息原文与最多 3 条 DS 扩展 query 分开计算向量命中，扩展命中不再回填成原文命中；cosine、BM25、多 query 支撑度与独立记忆 prior 均归一化到 `0～1`，生命周期权重不再混入相关性分。向量与 BM25 合池后保留到 reranker 配置容量（默认 30 条）交给 Qwen，按 `0.85 rerank + 0.10 recall + 0.05 prior` 统一排序，只有之后才按 `DYNAMIC_MEMORY_TOP_N`（默认 5）与动态 token budget 注入。物理淘汰仍是唯一召回生命周期出口，仍保存在动态层中的全部条目都进入候选资格判断，不再按独立 10 天有效期二次过滤 |
