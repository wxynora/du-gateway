...
| 生成图片工具 | `services/generated_image_tool.py`、`services/gateway_tools.py`、`services/chat_tools.py`、`routes/chat.py`、`routes/miniapp/media.py` | 仅在配置 `DU_IMAGE_GENERATION_URL` 时向模型注入 `generate_image`；工具使用独立 Images 上游生成一张 PNG/JPEG/WebP，原子保存到本机 `data/generated_images`，并通过无需 MiniApp 登录的只读 `/miniapp-api/generated-images/<image_id>` 返回长期缓存资源。工具循环结束后，QQ 与 SumiTalk 的流式/非流式响应从成功工具结果提取图片并随当前回复投递，正文不追加链接或 Markdown；同一 URL 在单轮去重。普通聊天上游、R2 与原生 App 协议不承担图片生成或存储 |
...
| 动态记忆检索 | `pipeline/pipeline.py`、`services/dynamic_memory_search.py`、`services/dynamic_memory_reranker.py` | DS 扩展 query 请求显式设置 `thinking.type=disabled`，`max_tokens=160` 只供三行短正文使用，定向回归为 `scripts/test_dynamic_memory_query_rewrite_payload.py`。当前消息原文与最多 3 条 DS 扩展 query 分开计算向量命中，扩展命中不再回填成原文命中；cosine、BM25、多 query 支撑度与独立记忆 prior 均归一化到 `0～1`，生命周期权重不再混入相关性分。向量与 BM25 合池后保留到 reranker 配置容量（默认 30 条）交给 Qwen，按 `0.85 rerank + 0.10 recall + 0.05 prior` 统一排序，只有之后才按 `DYNAMIC_MEMORY_TOP_N`（默认 5）与动态 token budget 注入。多模态当前 user 消息只拼接全部 `type=text` part；图片、音频等非文本 part 不进入关键词、DS、向量、BM25、rerank query 或召回调试 query，定向回归为 `scripts/test_dynamic_memory_recall_multimodal_query.py`。物理淘汰仍是唯一召回生命周期出口，仍保存在动态层中的全部条目都进入候选资格判断，不再按独立 10 天有效期二次过滤 |
